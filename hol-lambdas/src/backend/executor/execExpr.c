...
static void ExecInitFunc(ExprEvalStep *scratch, Expr *node, List *args,
						 ...
						 ExprState *state);
static void ExecInitLambdaFuncAlias(ExprEvalStep *scratch, 
									LambdaFuncAliasExpr *lfa, 
									Datum *resv,
									bool *resnull,
									ExprState *state);
static void ExecInitCallLambdaExpr(ExprEvalStep *scratch, 
									CallLambdaExpr *cle, 
									Datum *resv, 
									bool *resnull, 
									ExprState *state);
 ...
 * Mainly used in UDF context, for normal SQL lambda functions,
 * use the standard ExecInitExpr.
 */
ExprState *
ExecInitLambdaExpr(Node *node, bool fastLambda, bool buildDiff)
{
	...
	ExecCreateExprSetupSteps(state, (Node *)expr->expr); //MASSIVE GUESS, NOT TESTED, JUST USED
	...
	if (!fastLambda && state->parent)
	{
		paramList = (ParamListInfo)palloc0(offsetof(ParamListInfoData, params) +
										   list_length(expr->argnames) * sizeof(ParamExternData));
		paramList->numParams = list_length(expr->argnames);
...
void ExecCheckLambdaForMatrix(ExprState *expression)
{
	...
	for(int i = 0; i < expression->steps_len; i++) {
		if ((ExecEvalStepOp(expression, &(expression->steps[i])) == EEOP_FIELDSELECT) && expression->steps[i].d.fieldselect.resulttype == FLOAT8ARRAYOID)
		{
...
int* ExecGenerateIndexArray(LambdaExpr *lambda) {
	if(!lambda | !lambda->argTypesTupDesc)
		return NULL;
	int numTables = list_length(lambda->argnames);
	int *indexArray = palloc0(numTables * sizeof(int));
	int runningTally = 0;
	for(int i = 0; i < numTables; i++) {
		indexArray[i] = runningTally;
		TupleDesc inDesc = (TupleDesc)list_nth(lambda->argTypesTupDesc, i);
...
int ExecGetLambdaDerivativesLength(LambdaExpr *expr) {
	int numTables = list_length(expr->argnames);
	int runningTally = 0;
	for (int i = 0; i < numTables; i++)
	{
		TupleDesc inDesc = (TupleDesc)list_nth(expr->argTypesTupDesc, i);
		runningTally += inDesc->natts;
	}
	return runningTally;
}
 ...
 * 
 * This function serves as a single call to evaluate and derive a single lambda function for a single point, 
...
ExecLambdaDeriveSubtree(ExprState *state, int fetchIndex, Datum seed, Datum *derivatives) {
	...
	{
	case EEOP_FIELDSELECT: /*EEOP_FIELDSELECT*/
	{
	...
	case EEOP_FUNCEXPR: /*EEOP_FUNCEXPR*/
	{
		...
		{
			float8 x = DatumGetFloat8(state->steps[fetchIndex].d.func.fcinfo_data->args[0].value);
			float8 y = DatumGetFloat8(state->steps[fetchIndex].d.func.fcinfo_data->args[1].value);
		...
		case F_ATAN: /* float arcus tangens */
		{
			float8 x = DatumGetFloat8(state->steps[fetchIndex].d.func.fcinfo_data->args[0].value);
		...
		case F_ATAN2: /* float arcus tangens 2 (c: atan2) */
		{
			float8 x = DatumGetFloat8(state->steps[fetchIndex].d.func.fcinfo_data->args[0].value);
			float8 y = DatumGetFloat8(state->steps[fetchIndex].d.func.fcinfo_data->args[1].value);
		...
		case F_SOFTMAX_CCE: /* float softmax_ce */
		{
			Datum x = state->steps[fetchIndex].d.func.fcinfo_data->args[0].value; //nn-output vector
			Datum y = state->steps[fetchIndex].d.func.fcinfo_data->args[1].value; //labels-vector(needs no derivation)

			Datum newSeedX = matrix_mul_internal(seed, softmax_cce_derive(x, y), false, false); 
			Datum newSeedY = createScalar(0.0); //derivative to one-hot is not important, but needs derivation nonetheless
		...
		case F_SILU: /* float sigmoid rectified linear unit(silu) */
		{
			float8 x = DatumGetFloat8(state->steps[fetchIndex].d.func.fcinfo_data->args[0].value);
		...
		case F_TANH: /* float tangens hyperbolicus(tanh) */
		{
			float8 x = DatumGetFloat8(state->steps[fetchIndex].d.func.fcinfo_data->args[0].value);
		...
		case F_RELU: /* float rectified linear unit(relu) */
		{
			...
			if (x <= 0.0)
			{
				newSeedX = Float8GetDatum(0.0); //relu is undefined at 0, but one can make the choice to pick a number from [0,1], in this case 0
			}
			else
			{
				newSeedX = seed;
		...
		case F_MAT_MUL: /* array float8 matrix multiplication */
		{
			Datum x = state->steps[fetchIndex].d.func.fcinfo_data->args[0].value;
			Datum y = state->steps[fetchIndex].d.func.fcinfo_data->args[1].value;
		...
		case F_SILU_M: /* matrix element-wise silu */
		{
			Datum x = state->steps[fetchIndex].d.func.fcinfo_data->args[0].value;
		...
		case F_SIGMOID_M: /* matrix element-wise sigmoid */
		{
			Datum x = state->steps[fetchIndex].d.func.fcinfo_data->args[0].value;
		...
		case F_TANH_M: /* matrix element-wise tanh */
		{
			Datum x = state->steps[fetchIndex].d.func.fcinfo_data->args[0].value;
		...
		case F_RELU_M: /* matrix element-wise relu */
		{
			Datum x = state->steps[fetchIndex].d.func.fcinfo_data->args[0].value;
		...
		case F_MAT_ADD: /* array float8 matrix addition */
		{
			int startingPointForY = ExecLambdaDeriveSubtree(state, fetchIndex - 1, seed, derivatives);
			int stepIndexAfterX = ExecLambdaDeriveSubtree(state, startingPointForY, seed, derivatives);
			resultFetchIndex = stepIndexAfterX;
		...
		default:
		{
			ereport(ERROR, 
				...
				errmsg("Derive(Interpreted): current operator %d not supported, aborting...", state->steps[fetchIndex].d.func.finfo->fn_oid)));
			break;
		}
		}
		break;
	...
	default:
	{
		ereport(ERROR, 
			(errcode(ERRCODE_FEATURE_NOT_SUPPORTED), 
			errmsg("Derive(Interpreted): current step-opcode %d not recognized, aborting...", ExecEvalStepOp(state, &(state->steps[fetchIndex])))));
		break;
	}
	}
...
/*
 * Prepare a compiled expression for execution and derivation.  This has to be called for
 * every ExprState before it can be executed.
 *
 * NB: While this currently only calls ExecReadyInterpretedExpr(),
 * this will likely get extended to further expression evaluation methods.
				...
				Datum *resv, bool *resnull)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_LambdaFuncAliasExpr:
			{
				...
				ExecInitLambdaFuncAlias(&scratch, lfa, resv, resnull, state);
		...
		case T_CallLambdaExpr:
			{
				CallLambdaExpr *cle = (CallLambdaExpr *) node;

				ExecInitCallLambdaExpr(&scratch, cle, resv, resnull, state);
				ExprEvalPushStep(state, &scratch);
				break;
			}
		...
		case T_FieldSelect:
			{
				...
				scratch.d.fieldselect.rowcache.cacheptr = fselect->lambda;
			 ...
			 Oid inputcollid, ExprState *state)
{
	...
	foreach(lc, args)
	{
		...
		if (IsA(arg, Const) && !state->load_consts_explicitly)
		{
			...
			fcinfo->args[argno].value = PointerGetDatum(le);
			fcinfo->args[argno].isnull = false;
		}
		else
		{
			ExecInitExprRec(arg, state,
...
/*
 * Add expression step of a LambdaFuncAlias (replaced with CallLambdaExpr)
 */
static void
ExecInitLambdaFuncAlias(ExprEvalStep *scratch, 
	LambdaFuncAliasExpr *lfa, 
	Datum *resv,
	bool *resnull,
	ExprState *state)
{
	CallLambdaExprState *clestate;
	CallLambdaExpr	   *cle;
	ListCell		   *lc;
	int					i;
	...
	/*
	 * An LFA is a call to a lambda-valued argument of the currently executing
	 * lambda. The lambda datum lives in PARAM_EXEC slot lfa->fieldSelect_id(0-based).
	 * Dont build a fake Param node into CallLambdaExpr, just emit the param-load
	 * step straight into clestate->lambda_value.
	 */
	cle = makeNode(CallLambdaExpr);
	cle->lambda_expr = NULL;
	cle->args = lfa->args;
	cle->rettype = lfa->rettype;
	cle->rettypmod = -1;
	cle->retcollid = get_typcollation(lfa->rettype);
	cle->lambdatypmod = -1;
	cle->location = lfa->location;

	clestate = palloc0(sizeof(CallLambdaExprState));
	clestate->expr = cle;
	clestate->nargs = list_length(lfa->args);

	clestate->lambda_value = (Datum) 0;
	clestate->lambda_isnull = true;

	if (clestate->nargs > 0)
	{
		clestate->argvalues = palloc0(sizeof(Datum) * clestate->nargs);
		clestate->argnulls = palloc0(sizeof(bool) * clestate->nargs);
	}
	else
	{
		clestate->argvalues = NULL;
		clestate->argnulls = NULL;
	}

	clestate->cached_lambda_valid = false;
	clestate->cached_lambda_image = (Datum) 0;
	clestate->cached_body_state = NULL;
	clestate->cached_lambda_cxt = NULL;

	/* direct PARAM_EXEC load for the lambda-valued arg, without pretending its cle->lambda_expr */
	scratch->opcode = EEOP_PARAM_EXEC;
	scratch->resvalue = &clestate->lambda_value;
	scratch->resnull = &clestate->lambda_isnull;
	scratch->d.param.paramid = lfa->fieldSelect_id;		/* 0-based */
	scratch->d.param.paramtype = LAMBDAOID;
	ExprEvalPushStep(state, scratch);

	/* normal arg evaluation steps */
	i = 0;
	foreach(lc, lfa->args)
	{
		Expr *arg = (Expr *) lfirst(lc);
		...
		ExecInitExprRec(arg,
						state,
						&clestate->argvalues[i],
						&clestate->argnulls[i]);

		i++;
	}

	/* and the actual call step */
	scratch->opcode = EEOP_CALLLAMBDAEXPR;
	scratch->resvalue = resv;
	scratch->resnull = resnull;
	scratch->d.calllambdaexpr.cle_state = clestate;
}

/*
 * Add expr step performing the operations of the referenced lambda_expr
 */
static void 
ExecInitCallLambdaExpr(ExprEvalStep *scratch, 
						CallLambdaExpr *cle, 
						Datum *resv, 
						bool *resnull, 
						ExprState *state)
{
	...
	Assert(cle != NULL);
	Assert(cle->lambda_expr != NULL);

	clestate = palloc0(sizeof(CallLambdaExprState));

	clestate->expr = cle;
	clestate->nargs = list_length(cle->args);
	...
	/* runtime cache, init here so the lazy-compile path stays clean */
	clestate->cached_lambda_valid = false;
	clestate->cached_lambda_image = (Datum) 0;
	clestate->cached_body_state = NULL;
	clestate->cached_lambda_cxt = NULL;

	/* eval the expr producing the lambda value(e.g. Var(l.f)) -> clestate->lambda_value */
	ExecInitExprRec(cle->lambda_expr,
					state,
					&clestate->lambda_value,
					&clestate->lambda_isnull);

	/* eval the normal call args -> clestate arrays */
	i = 0;
	foreach(lc, cle->args)
	{
		Expr *arg = (Expr *) lfirst(lc);
	...
	/*
	 * the actual lambda-call step. At runtime it consumes clestate->lambda_value
	 * and clestate->argvalues, then writes the result into resv/resnull for the parent.
	 */
	scratch->opcode = EEOP_CALLLAMBDAEXPR;
	scratch->resvalue = resv;
	scratch->resnull = resnull;
