...
ExecInterpExpr(ExprState *state, ExprContext *econtext, bool *isnull)
{
	...
	static const void *const dispatch_table[] = {
		...
		&&CASE_EEOP_CONST,
		&&CASE_EEOP_LAMBDAFUNCALIAS,
		&&CASE_EEOP_CALLLAMBDAEXPR,
		&&CASE_EEOP_FUNCEXPR,
	...
	EEO_SWITCH()
	{
		...
		/*
		 * The execution of a LambdaFuncAlias is decievingly simple,
		 * only a simple jump has to be made to the steps, that have been
		 * appended to the end of the steps array and the jumpdone parameter
		 * has been set by the injector.
		 * 
		 * NOTICE: Currently, reaching this should not be possible!!!!
		 */
		EEO_CASE(EEOP_LAMBDAFUNCALIAS)
		{
			ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED), errmsg("You reached a LFA during execution ... how?")));
			EEO_JUMP(op->d.lambdafuncalias.jumpdone);
		}

		/*
		 * CallLambdaExpr loads its Var arg holding the lambda function
		 * fetched from the table, and compiles it into working EEOP_steps.
		 * 
		 * Handles args like any other funccall node.
		 */
		EEO_CASE(EEOP_CALLLAMBDAEXPR)
		{
			ExecEvalCallLambdaExpr(state, op, econtext);
...
/*
 * Evaluate EEOP_CALLLAMBDAEXPR
 */
void
ExecEvalCallLambdaExpr(ExprState *state, ExprEvalStep *op,
					   ExprContext *econtext)
{
	CallLambdaExprState *clestate = op->d.calllambdaexpr.cle_state;
	SerializedLambda *serialized;
	LambdaExpr *le;
	ExprState *body_state;
	ParamExecData *save_params;
	...
	/*
	 * Partial application("_" holes): instead of evaluating the body to a scalar,
	 * bake the supplied args into a copy of the mother and return the residual
	 * lambda value. Held positions stay open, lambda_partial_apply reindexes them
	 * to a dense 0..k-1 range.
	 */
	if (clestate->expr->heldargs != NULL)
	{
		SerializedLambda *ser;
		LambdaExpr	   *mother;
		LambdaExpr	   *residual;
		SerializedLambda *res_ser;
		Datum		   *full_vals;
		bool		   *full_nulls;
		int				N;
		int				p;
		...
		ser = (SerializedLambda *) PG_DETOAST_DATUM(clestate->lambda_value);
		mother = lambda_deserialize(ser);
		if (mother == NULL || mother->expr == NULL)
			elog(ERROR, "deserialized lambda has no expression body");

		N = list_length(mother->argnames);
		full_vals = palloc0(sizeof(Datum) * (N > 0 ? N : 1));
		full_nulls = palloc0(sizeof(bool) * (N > 0 ? N : 1));

		/* spread the filled args (mother-order) back over the full position set */
		fcursor = 0;
		for (p = 0; p < N; p++)
		{
			if (bms_is_member(p, clestate->expr->heldargs))
			{
				full_nulls[p] = true;	/* held: no value */
			}
			else
			{
				if (fcursor >= clestate->nargs)
					elog(ERROR, "partial apply: fewer filled args than expected");
				full_vals[p] = clestate->argvalues[fcursor];
				full_nulls[p] = clestate->argnulls[fcursor];
				fcursor++;
			}
		}

		residual = lambda_partial_apply(mother, clestate->expr->heldargs,
										full_vals, full_nulls, N);

		residual->expr = (Expr *) eval_const_expressions(NULL, (Node *) residual->expr);

		res_ser = lambdaExprToSerialLambda(residual);

		*op->resvalue = PointerGetDatum(res_ser);
		*op->resnull = false;
		return;
	}

	/*
	 * Re-assemble only when the lambda *combination* changes: the mother lambda
	 * value, or(on the hoisting path) any of the lambda-valued args injected into
	 * the cached body. Atomic args never trigger it - theyre bound per row via
	 * PARAM_EXEC below.
	 */
	lambda_changed = !clestate->cached_lambda_valid ||
		!datum_image_eq(clestate->lambda_value,
						clestate->cached_lambda_image,
						false,
						-1);

	if (!lambda_changed && clestate->hoisted)
	{
		for (i = 0; i < clestate->n_cached_lambda_args; i++)
		{
			int			idx = clestate->cached_lambda_arg_idx[i];

			if (clestate->argnulls[idx] ||
				!datum_image_eq(clestate->argvalues[idx],
								clestate->cached_lambda_arg_image[i],
								false,
								-1))
			{
				lambda_changed = true;
	...
	if (lambda_changed)
	{
		...
		if (clestate->cached_lambda_cxt != NULL)
			MemoryContextDelete(clestate->cached_lambda_cxt);

		clestate->cached_lambda_cxt =
			AllocSetContextCreate(econtext->ecxt_per_query_memory,
								  "CallLambdaExpr cache",
								  ALLOCSET_DEFAULT_SIZES);

		MemoryContextSwitchTo(clestate->cached_lambda_cxt);

		serialized = (SerializedLambda *) PG_DETOAST_DATUM(clestate->lambda_value);
		le = lambda_deserialize(serialized);

		if (le == NULL || le->expr == NULL)
			elog(ERROR, "deserialized lambda has no expression body");

		/*
		 * If the mother is higher order(calls a lambda-valued input) and hoisting
		 * is on, flatten it once for this combination via the scalar injector and
		 * compile a single first-order ExprState. Else keep the per-row body:
		 * first-order lambdas, or - with hoisting off - the legacy nested
		 * CallLambdaExpr interpretation path.
		 */
		do_hoist = custom_lambda_hoisting_enabled() && lambda_body_has_lfa(le);

		if (do_hoist)
		{
			LambdaExpr *injected;
			int		   *idxbuf = palloc(sizeof(int) * clestate->nargs);
			int			nfunc = lambda_scalar_func_inputs(le,
														  clestate->argvalues,
														  clestate->argnulls,
														  clestate->nargs,
														  idxbuf);

			injected = lambda_injector_scalar(le,
											  clestate->argvalues,
											  clestate->argnulls,
											  clestate->nargs);

			body_state = ExecInitExpr((Expr *) injected->expr, state->parent);

			clestate->hoisted = true;
			clestate->cached_lambda_arg_idx = idxbuf;
			clestate->n_cached_lambda_args = nfunc;
			clestate->cached_lambda_arg_image = palloc0(sizeof(Datum) * nfunc);
			for (i = 0; i < nfunc; i++)
				clestate->cached_lambda_arg_image[i] =
					datumCopy(clestate->argvalues[idxbuf[i]], false, -1);
		}
		else
		{
			body_state = ExecInitExpr((Expr *) le->expr, state->parent);

			clestate->hoisted = false;
			clestate->cached_lambda_arg_idx = NULL;
			clestate->cached_lambda_arg_image = NULL;
			clestate->n_cached_lambda_args = 0;
		}

		clestate->cached_lambda_valid = true;
		clestate->cached_lambda_image = datumCopy(clestate->lambda_value, false, -1);
		clestate->cached_body_state = body_state;

		MemoryContextSwitchTo(oldcxt);
	}
	else
		body_state = clestate->cached_body_state;

	/*
	 * Dont install a tiny nargs-sized ParamExecData array: higher-order bodies can
	 * have nested PARAM_EXEC refs, and out-of-bounds paramid access here looks like
	 * random executor/JIT corruption.
	 */
	lambda_params = palloc0(sizeof(ParamExecData) * LAMBDA_PARAM_EXEC_SLOTS);

	if (clestate->nargs > LAMBDA_PARAM_EXEC_SLOTS)
		elog(ERROR, "too many lambda arguments: %d", clestate->nargs);

	/*
	 * Preserve the query's PARAM_EXEC values so the lambda body doesnt clobber the
	 * surrounding executor param namespace. Copy from the query's own param array
	 * (es_param_exec_vals) and only as many slots as actually exist
	 * (list_length(paramExecTypes)) - the old code blindly memcpy'd
	 * LAMBDA_PARAM_EXEC_SLOTS entries out of ecxt_param_exec_vals(sized to the much
	 * smaller query param count), reading way past the end.
	 */
	if (econtext->ecxt_estate != NULL &&
		econtext->ecxt_estate->es_param_exec_vals != NULL &&
		econtext->ecxt_estate->es_plannedstmt != NULL)
	{
		int			n_src = list_length(econtext->ecxt_estate->es_plannedstmt->paramExecTypes);

		if (n_src > LAMBDA_PARAM_EXEC_SLOTS)
			n_src = LAMBDA_PARAM_EXEC_SLOTS;
		if (n_src > 0)
			memcpy(lambda_params,
				   econtext->ecxt_estate->es_param_exec_vals,
				   sizeof(ParamExecData) * n_src);
	}

	for (i = 0; i < clestate->nargs; i++)
	{
		lambda_params[i].execPlan = NULL;
		lambda_params[i].value = clestate->argvalues[i];
		lambda_params[i].isnull = clestate->argnulls[i];
	}

	save_params = econtext->ecxt_param_exec_vals;
	econtext->ecxt_param_exec_vals = lambda_params;

	result = ExecEvalExprSwitchContext(body_state, econtext, &isnull);

	econtext->ecxt_param_exec_vals = save_params;
...
ExecEvalFieldSelect(ExprState *state, ExprEvalStep *op, ExprContext *econtext)
{
	...
	else
	{
		...
		if(op->d.fieldselect.lambda) {
			tupDesc = (TupleDesc) op->d.fieldselect.rowcache.cacheptr;
		} else {
			tupType = HeapTupleHeaderGetTypeId(tuple);
			tupTypmod = HeapTupleHeaderGetTypMod(tuple);
