				...
				bool *isNull)
{
	...
	*isNull = false;
	...
	if (subLinkType == CURSOR_SUBLINK) //If we are handling a subquery for lambda function inputs, we quit before postgres flags this as an error and return a ptr to the planstate
	{
		 ...
		 slot = ExecProcNode(planstate))
	{
		...
		if (subLinkType == FUNC_SUBLINK) //only found in context with lambda functions and tablefunctions
		{
	...
	{
		/* again, only found in context with lambda functions */
...
ExecSetParamPlan(SubPlanState *node, ExprContext *econtext)
{
	...
	bool		found = false;
	ArrayBuildStateAny *astate = NULL;
	Tuplestorestate *tupstorestate; //only needed in context with table functions containing lambdafunctions
	TypedTuplestore *tts;
	...
	else if (subLinkType == FUNC_SUBLINK) //only found in context with tablefunctions containing lambdas
	{
		/* There can be only one setParam... */
		int			paramid = linitial_int(subplan->setParam);
		ParamExecData *prm = &(econtext->ecxt_param_exec_vals[paramid]);
