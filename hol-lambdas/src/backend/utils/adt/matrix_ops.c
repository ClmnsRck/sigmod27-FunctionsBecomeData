...
#pragma omp parallel for
    ...
    ps1 = (float8 *)ARR_DATA_PTR(a1);
    ps2 = (float8 *)ARR_DATA_PTR(a2);
...
#pragma omp parallel for
    ...
    ps1 = (Datum *)ARR_DATA_PTR(a1);
    ps2 = (Datum *)ARR_DATA_PTR(a2);
#pragma omp parallel for
...
#pragma omp parallel for
    ...
    ret_data = (Datum *)ARR_DATA_PTR(ret);
    a_data = (Datum *)ARR_DATA_PTR(a);
    b_data = (Datum *)ARR_DATA_PTR(b);
...
Datum mat_sub_mm(PG_FUNCTION_ARGS)
{
    ...
    retData = (Datum *)ARR_DATA_PTR(ret);
    a1Data = (Datum *)ARR_DATA_PTR(a1);
    a2Data = (Datum *)ARR_DATA_PTR(a2);
...
Datum matrix_transpose_internal(Datum MatA)
{
    ...
    if (isScalar(org))
    {
        PG_RETURN_ARRAYTYPE_P(copyArray(MatA));
    }
    if (ARR_NDIM(org) == 1)
    {
        dims[0] = 1;
    ...
    ret = initResult(2, dims, lbs);
    A = (Datum *)ARR_DATA_PTR(org);
    B = (Datum *)ARR_DATA_PTR(ret);
...
Datum mat_apply_gradient(Datum weights, Datum derivatives, float8 learning_rate, int batch_size)
{
    ...
    if (isScalar(weights_a))
    {
        ...
        data = (Datum *) ARR_DATA_PTR(weights_a);
    ...
    ndims = ARR_NDIM(weights_a);
    dims = ARR_DIMS(weights_a);
    ...
    data = (Datum *)ARR_DATA_PTR(weights_a);
    derivatives_data = (Datum *)ARR_DATA_PTR(derivatives_a);
...
Datum index_max(PG_FUNCTION_ARGS)
{
    ...
    data = (Datum *)ARR_DATA_PTR(input);
    max_value = DatumGetFloat8(data[0]);
...
Datum softmax(PG_FUNCTION_ARGS)
{
    ...
    data = (float8 *)ARR_DATA_PTR(array);
...
Datum softmax_cce_internal(Datum inputs_in, Datum labels_in)
{
    ...
    data = (float8 *)ARR_DATA_PTR(input);
    label_data = (float8 *)ARR_DATA_PTR(labels);

    length = ArrayGetNItems(ARR_NDIM(input), ARR_DIMS(input));
    ...
    result = (float8 *)palloc0(length * sizeof(float8));
...
Datum softmax_cce_derive(Datum inputs_in, Datum labels_in)
{
    ...
    input = (float8 *)ARR_DATA_PTR(input_arr);
    labels = (float8 *)ARR_DATA_PTR(labels_arr);

    length = ArrayGetNItems(ARR_NDIM(input_arr), ARR_DIMS(input_arr));
    ...
    result = (float8 *)ARR_DATA_PTR(result_arr);
...
ArrayType *initResult(int ndims, int *dims, int *lbs)
{
    ...
    ret = (ArrayType *)palloc_extended(nbytes, (MCXT_ALLOC_ZERO));
...
ArrayType *copyArray(Datum orgArray)
{
    ...
    original = DatumGetArrayTypeP(orgArray);
    ndims = ARR_NDIM(original);
    dims = ARR_DIMS(original);
    lbs = ARR_LBOUND(original);
    // create new array
    nelems = ArrayGetNItems(ndims, dims);
    nbytes = nelems * ALIGNOF_DOUBLE;
    nbytes += ARR_OVERHEAD_NONULLS(ndims);
    ret = (ArrayType *)palloc_extended(nbytes, (MCXT_ALLOC_ZERO));
    SET_VARSIZE(ret, nbytes);
    ret->ndim = ndims;
...
Datum createArray(int *dims, const float8 value, const bool identityMatrix)
{
    ...
    ret = initResult(2, dims, lbs);
    data = (Datum *) ARR_DATA_PTR(ret);
    if (identityMatrix && dims[0] == dims[1])
    {
#pragma omp parallel for
...
void matrixPrint(ArrayType *in)
{
    ...
    data_ptr = (float8 *)ARR_DATA_PTR(in);
