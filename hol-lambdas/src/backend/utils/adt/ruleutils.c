...
typedef struct
{
	...
	LambdaExpr *cur_lambda;		/* holds information about the lambda_context we might be in currently */
} deparse_context;
...
static void get_call_lambda_expr(CallLambdaExpr *node, deparse_context *context,
								 bool showimplicit);
...
get_parameter(Param *param, deparse_context *context)
{
	...
	/*
	 * Lambda functions internally represent leaf nodes as ParamExec nodes.
	 * This means, that if cur_lambda is set, simply look up the matching 
	 * argnames and return.
	 * 
	 * This only works on standalone lambdas, hence the PARAM_EXEC (sad noises)
	 */
	if(context->cur_lambda && param->paramkind == PARAM_EXEC)
	{
		LambdaExpr *le = (LambdaExpr *) context->cur_lambda;
		appendStringInfoString(context->buf, list_nth_node(String, le->argnames, param->paramid)->sval);
		return;
	}
...
/*
 * Append a ColumnRef-style reference to a lambda input column, such as "a.x".
 *
 * Assumes the FieldSelect node has a Param child of kind PARAM_EXTERN.
 * Requires that the deparse_context has a valid cur_lambda field set.
 */
static void
append_lambda_columnref(FieldSelect *fs, deparse_context *context, StringInfo buf)
{
	Param *param;
	LambdaExpr *lambda;
	int param_idx;
	...
	List *row_type_list;
	List *row_fields;
	LambdaInputParam *field;
	String *field_name;

	Assert(fs && IsA(fs, FieldSelect));
	Assert(context && context->cur_lambda != NULL);
	Assert(IsA(fs->arg, Param));

	param = (Param *) fs->arg;
	lambda = context->cur_lambda;

	param_idx = param->paramid - 1;  /* lambda paramid is 1-based */
	field_idx = fs->fieldnum - 1;    /* fieldnum is also 1-based */
	row_alias_list = lambda->argnames;
	row_alias = list_nth_node(String, row_alias_list, param_idx);

	/* Safety check */
	if (param_idx >= list_length(lambda->argnames))
		ereport(ERROR,
		        (errcode(ERRCODE_SYNTAX_ERROR),
		         errmsg("parameter index out of range for lambda expression"),
		         errdetail("Expected at most %d parameters, found %d",
		                   list_length(lambda->argnames), param->paramid)));

    if (lambda->lambdakind == LAMBDA_SCALAR_CONTEXT)
	{
		String *arg_name = list_nth_node(String, row_alias_list, field_idx);
		appendStringInfo(buf, "%s", quote_identifier(strVal(arg_name)));
		return;
	}

	if (param_idx >= list_length(lambda->argtypes))
		ereport(ERROR,
		        (errcode(ERRCODE_SYNTAX_ERROR),
		         errmsg("parameter types missing for lambda expression")));

	row_type_list = lambda->argtypes;
	row_fields = list_nth_node(List, row_type_list, param_idx);

	if (field_idx >= list_length(row_fields))
		ereport(ERROR,
		        (errcode(ERRCODE_SYNTAX_ERROR),
		         errmsg("field index out of range for lambda row argument"),
		         errdetail("Expected at most %d fields, found %d",
		                   list_length(row_fields), fs->fieldnum)));

	field = list_nth_node(LambdaInputParam, row_fields, field_idx);
	field_name = linitial_node(String, field->name);

	/* Print as: rowalias.fieldname */
	appendStringInfo(buf, "%s.%s",
	                 quote_identifier(strVal(row_alias)),
	                 quote_identifier(strVal(field_name)));
}

/*
 * Get_lambda_expr
 * helper function to print the body of a lambda-function and its inputs in a
 * digestable and re-inputable way.
 */
static void
get_lambda_expr(LambdaExpr *node, deparse_context *context, bool showimplicit)
{
	LambdaExpr *old;
	old = context->cur_lambda;
	context->cur_lambda = node;
	appendStringInfo(context->buf, "lambda(");

    printLambdaInputsWithType(node, context->buf);
	
    appendStringInfoString(context->buf, ")(");

    get_rule_expr((Node *) node->expr, context, showimplicit);

	appendStringInfoChar(context->buf, ')');
	context->cur_lambda = old;
}

/*
 * Get_lambda_func_alias_expr
 * helper function to print the Function-call inside a lambda expression to
 * one of its own function-parameters. These are aliased outside of UDFs
 */
static void
get_lambda_func_alias_expr(LambdaFuncAliasExpr *node, deparse_context *context, bool showimplicit)
{
	ListCell *lc;

    appendStringInfo(context->buf, "%s(", NameListToString(node->name));
	foreach(lc, node->args)
	{
		get_rule_expr((Node *) lfirst(lc), context, showimplicit);
		if (lnext(node->args, lc) != NULL)
        	appendStringInfoString(context->buf, ", ");
	}
	appendStringInfoChar(context->buf, ')');
}
...
/*
 * Get_call_lambda_expr
 * helper function to print direct calls to lambda-valued expressions outside
 * of lambda bodies.
 */
static void
get_call_lambda_expr(CallLambdaExpr *node, deparse_context *context, bool showimplicit)
{
	ListCell   *lc;

	if (node->lambda_expr == NULL)
		ereport(ERROR,
				(errcode(ERRCODE_SYNTAX_ERROR),
				 errmsg("CallLambdaExpr is missing its lambda expression")));

	get_rule_expr((Node *) node->lambda_expr, context, showimplicit);
	appendStringInfoChar(context->buf, '(');
			  ...
			  bool showimplicit)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_LambdaExpr:
			get_lambda_expr((LambdaExpr *) node, context, showimplicit);
			break;

		case T_LambdaFuncAliasExpr:
			get_lambda_func_alias_expr((LambdaFuncAliasExpr *) node, context, showimplicit);
			break;

		case T_CallLambdaExpr:
			get_call_lambda_expr((CallLambdaExpr *) node, context, showimplicit);
			break;
		...
		case T_SubPlan:
			{
				...
				switch (subplan->subLinkType)
				{
					...
					case ARRAY_SUBLINK:
						...
						break;
					case FUNC_SUBLINK:
						/* lambda-body sublink (custom); just reference it */
						appendStringInfoString(buf, "LAMBDA(");
						break;
					case CURSOR_SUBLINK:
						/* cursor sublink (custom); just reference it */
						appendStringInfoString(buf, "CURSOR(");
						break;
					case CTE_SUBLINK:
		...
		case T_FieldSelect:
			{
				...
				if (context->cur_lambda && IsA(arg, Param))
				{
					append_lambda_columnref(fselect, context, buf);
					break;
				}
