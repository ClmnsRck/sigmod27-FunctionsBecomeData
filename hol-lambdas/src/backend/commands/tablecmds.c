...
BuildDescForRelation(const List *columns)
{
	...
	foreach(l, columns)
	{
		...
		/*
		 * Lambda columns must carry a stored lambda signature. Normal CREATE TABLE
		 * gives us the raw signature AST on entry->typeName, store it in pg_lambdasig
		 * here. Generated ColumnDefs(CTAS) may already have it resolved into typmod,
		 * so keep that instead of demanding raw syntax again.
		 */
		if (atttypid == LAMBDAOID)
		{
			LambdaSignature *sig;

			if (entry->typeName->lambda_signature != NULL)
			{
				sig = castNode(LambdaSignature, entry->typeName->lambda_signature);
				atttypmod = LambdaSigCreateFromSignature(sig);
			}
			else if (atttypmod < 0)
				ereport(ERROR,
						(errcode(ERRCODE_INVALID_TABLE_DEFINITION),
						errmsg("lambda column \"%s\" must specify a signature",
								attname)));
		}
