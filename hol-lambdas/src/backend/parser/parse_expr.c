...
static Node *transformLambdaExpr(ParseState *pstate, LambdaExpr *l);
...
transformExprRecurse(ParseState *pstate, Node *expr)
{
	...
	switch (nodeTag(expr))
	{
		...
		case T_LambdaExpr:
			result = transformLambdaExpr(pstate, (LambdaExpr *) expr);
			break;
		...
		case T_SetToDefault:
			...
			/*
			 * "_" is only meaningful as a direct lambda-call arg, where
			 * transformFuncCall passes it through untransformed. Reaching here means
			 * it was written somewhere illegal -> reject.
			 */
		case T_HolePlaceholder:
			ereport(ERROR,
					(errcode(ERRCODE_SYNTAX_ERROR),
					 errmsg("\"_\" is only allowed as an argument of a lambda call"),
					 parser_errposition(pstate,
										((HolePlaceholder *) expr)->location)));
			break;
			 ...
			 * 
			 * FuncAliases in Lmabda expressions also only ever get injected in 
			 * the transformer, so this is just good practice.
			 */
		case T_CaseTestExpr:
		case T_Var:
...
/*
 * Helper routine the check LambdaColumnRefs outside of UDFs(see LambdaColumnRefHook for that)
 * 
 * Outside of an UDF, the parser checks the names and maps them one-to-one to the passed arg
 * names. Because lambda functions in columns of tables are restricted to a specific signature,
 * said signature can be used to check for type-validity.
 */
Node *LambdaColumnRefHookOutsideUDF(ParseState *pstate, ColumnRef *colref)
{
	String *varName;
	Param* param;
	...
	varName = linitial_node(String, colref->fields);

	idx1 = list_index(pstate->p_current_lambda->argnames, varName);

	if (idx1 < 0 || idx1 >= list_length(((LambdaExpr *)pstate->p_current_lambda)->argnames))
	{
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_COLUMN_REFERENCE),
				 errmsg("variable not found in lambda expressions signature!"),
				 parser_errposition(pstate,
									(colref->location))));
	}
	...
	attr = list_nth_node(LambdaSigType, sig->argtypes, idx1);
	...
	if(attr->is_function)
	{
		...
		param->paramid = idx1;              /* 0-based */
		param->paramtype = LAMBDAOID;
		param->paramtypmod = -1;            /* ideally signature typmod of attr */
		param->paramcollid = InvalidOid;
		param->location = colref->location;
		param->lambda = true;
	...
	param->paramkind = PARAM_EXEC;
	param->paramid = idx1; //PARAM_EXEC starts at 0, because of course they do
	param->paramtype = attr->typeoid; //save because attr is atomic
	param->paramcollid = InvalidOid;
	param->location = colref->location;
	param->lambda = true;
...
transformFuncCall(ParseState *pstate, FuncCall *fn)
{
	...
	foreach(args, fn->args)
	{
		...
		/*
		 * LambdaExpr args get resolved in ParseFuncOrColumn, and a held "_" must
		 * reach its lambda-call branch raw. Both pass through untransformed -
		 * transformExprRecurse rejects "_" everywhere else, so this is the only
		 * spot a hole can survive.
		 */
		if (IsA(argnode, LambdaExpr) || IsA(argnode, HolePlaceholder))
...
transformSubLink(ParseState *pstate, SubLink *sublink)
{
	...
	switch (pstate->p_expr_kind)
	{
		...
		case EXPR_KIND_GENERATED_COLUMN:
			...
			break;
		case EXPR_KIND_LAMBDA_EXPRESSION:
			err = _("cannot use subquery in LAMBDA expressions");
			break;
...
/*
 * Lambdas outside of UDFs have no inputs, so type-validation needs to be done through
 * a ColumnRefHook that switches ColumnRefs with Param_extern nodes, such that they can
 * be accurately type-checked, based on the provided type-signature of the referenced argument
 */
static Node *
transformLambdaExpr(ParseState *pstate, LambdaExpr *l)
{
	PreParseColumnRefHook parent_columnRef_hook;
	LambdaExpr *parent_lambda;
	Expr *transformed_expr_body;
	LambdaContext * l_ctx;

	/* store the current lambda_scope, if present */
	parent_columnRef_hook = pstate->p_pre_columnref_hook;
	parent_lambda = pstate->p_current_lambda;

	if(parent_lambda || pstate->p_expr_kind == EXPR_KIND_LAMBDA_EXPRESSION) {
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				 errmsg("Declaration and definition of new lambda-function inside of another lambda-function!")));
	}

	if(pstate->p_lambdacontext == NULL)
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				 errmsg("Lambda function outside of an UDF found without type information/matchiing colum to provide as such!")));

	l_ctx = pstate->p_lambdacontext;

	pstate->p_pre_columnref_hook = LambdaColumnRefHookOutsideUDF;
	pstate->p_current_lambda = l;

	/* this lambda value's own signature typmod (pg_lambdasig key) */
	l->lambdatypmod = l_ctx->targettypmod;

	/* transform Expr-Body */
	transformed_expr_body = (Expr *) transformExpr(pstate, (Node *) l->expr, EXPR_KIND_LAMBDA_EXPRESSION);
	l->expr = transformed_expr_body;

	l->rettype = exprType((Node *) l->expr);
	l->rettypmod = exprTypmod((Node *) l->expr);

	/*
	 * Classify the (now transformed) body for L3 optimizability. Done once here
	 * while the fully-resolved Expr tree is in hand, cached on the node so the
	 * executor never re-derives it. See lambda_expr_is_l3_safe().
	 */
	l->l3_safe = lambda_expr_is_l3_safe(l);

	/* reset to old scope */
	pstate->p_current_lambda = parent_lambda;
	pstate->p_pre_columnref_hook = parent_columnRef_hook;

	return (Node *) makeLambdaConst(pstate, l);
}
