...
static Node *LookUpFuncNameAsLambda(ParseState *pstate, List *funcname, ParseLoc location);

Node *
LambdaColumnRefHook(ParseState *pstate, ColumnRef *colref)
{
	String *colName, *rowAlias;
	Param* param;
	FieldSelect *fs;
	LambdaInputParam *arg_type;
	LambdaFuncAliasExpr * lfa;
	TupleDesc input_of_lambda;
	Form_pg_attribute attr;

	int idx1 = -1, idx2 = -1;

	if(unlikely(list_length(colref->fields) != 2))
	{
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_COLUMN_REFERENCE),
				 errmsg("[LambdaColumRefHook]: invalid column reference inside lambda expression"),
				 errhint("Use full name of variable declared in func-signature!"),
				 parser_errposition(pstate,
									(colref->location))));
	}

	rowAlias = linitial_node(String, colref->fields);
	colName = lsecond_node(String, colref->fields);

	idx1 = list_index(pstate->p_current_lambda->argnames, rowAlias);

	if (unlikely(idx1 < 0 || idx1 >= list_length(pstate->p_current_lambda->argTypesTupDesc)))
	{
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_COLUMN_REFERENCE),
				 errmsg("[LambdaColumRefHook]: illegal rowAlias \"%s\" in top-level lambda expression", strVal(rowAlias)),
				 parser_errposition(pstate,
									(colref->location))));
	}

	input_of_lambda = (TupleDesc) list_nth(pstate->p_current_lambda->argTypesTupDesc, idx1);

	for (idx2 = 0; idx2 < input_of_lambda->natts; idx2++)
	{
		attr = TupleDescAttr(input_of_lambda, idx2);
	...
	if (unlikely(idx2 >= input_of_lambda->natts))
	{		
		ereport(ERROR, 
				(errcode(ERRCODE_INVALID_COLUMN_REFERENCE),
				 errmsg("[LambdaColumRefHook]: column \"%s\" does not exist", strVal(colName)),
				 parser_errposition(pstate,
									(colref->location))));
	} 

	arg_type = get_input_param_from_name(pstate->p_current_lambda, colref->fields);

	if(unlikely(!arg_type))
	{
		ereport(ERROR, 
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("[LambdaColumRefHook]: input \"%s\".\"%s\" does not exist", strVal(rowAlias), strVal(colName)),
				 parser_errposition(pstate,
									(colref->location))));
	} else if(!arg_type->isAtomic)
	{
		/*  
		* If a ColRef turns out to be a func, the Lambda function was passed through another call
		* meaning we have no arguments and only know our type and input through context of the
		* current lambda.
		*/
		lfa = makeNode(LambdaFuncAliasExpr);
		lfa->name = list_copy_deep(colref->fields);
		lfa->rettype = arg_type->type;
		lfa->args = NIL; 
		lfa->param_id = idx1 + 1;
		lfa->fieldSelect_id = idx2 + 1;
		lfa->uniqueness = -1;
		lfa->location = colref->location;
		return (Node *) lfa;
	}
				  ...
				  Node *last_srf, FuncCall *fn, bool proc_call, int location)
{
	...
	Node	   *first_arg = NULL;
	int			nargs;
	int			nholes = 0;		/* count of "_" held argument positions */
	int			nargsplusdefs;
	...
	List *tmp_funcname = funcname;
	LambdaInputParam *matching_LambdaInput = NULL;
	LambdaSigType *matching_LambdaSigType = NULL;
	int rowIdx_of_match = 0, colIdx_of_match = 0;
	Node *possibleLambdaInNamespaceMatch = NULL;
	...
	foreach(l, fargs)
	{
		...
		/*
		 * A held "_" has no value, so no type to read(exprType() would fail). Use
		 * an UNKNOWNOID sentinel - the lambda lookup is name-based and never looks
		 * at it - and let the FUNCDETAIL_LAMBDA branch take the real type from the
		 * matched signature.
		 */
		if (IsA(arg, HolePlaceholder))
		{
			nholes++;
	...
	/*
	 * If we are in a lambdaExpr body, ignore failed lookups, but throw
	 * an error, if we have a collision in the namespace. Normally, the 
	 * namespace is three parts, not 2 like in the lambda-context, but
	 * we can never be to careful.
	 * If no collision was found, probe the inputs of the lambda for a 
	 * input, that is a function that recieves the rettypes of the args-nodes.
	 * 
	 * Because the name-lookup for some reason does not have an implementation, 
	 * that also passes the ParseState down from the caller, a magic prefix is
	 * prepended to the funcname list, and func_get_detail knows to look out for that.
	 * (Not beautiful, but the quickest and most non-disruptive method to pass info
	 * to func_get_detail, without changing its signature or implementing a superfluous
	 * second implementation)
	 */
	if (pstate->p_expr_kind == EXPR_KIND_LAMBDA_EXPRESSION &&
		pstate->p_current_lambda != NULL)
	{
		if(pstate->p_current_lambda->lambdakind == LAMBDA_ROW_CONTEXT)
		{
			String *prefix;
			String *field;
			int arg_idx;

			Assert(list_length(funcname) == 2 && IsA(linitial(funcname), String) && IsA(lsecond(funcname), String));
			prefix = linitial_node(String, funcname);
			field  = lsecond_node(String, funcname);

			/* Find index of prefix in argnames */
			arg_idx = list_index(pstate->p_current_lambda->argnames, prefix);

			if (arg_idx >= 0)
			{
				List *params = (List *) list_nth(pstate->p_current_lambda->argtypes, arg_idx);
				ListCell *lc;
				int colIdx = 0;

				foreach(lc, params)
				{
					LambdaInputParam *param = (LambdaInputParam *) lfirst(lc);
					if (param->name &&
						list_length(param->name) == 1 &&
						IsA(linitial(param->name), String))
					{
						String *pname = linitial_node(String, param->name);
						if (strcmp(pname->sval, field->sval) == 0)
						{
							/* Both parts of funcname match lambda inputs */
							tmp_funcname = lcons(makeString(LAMBDA_PASS_TO_FUNC_DETAIL), funcname);
							matching_LambdaInput = param;
							rowIdx_of_match = arg_idx;
							colIdx_of_match = colIdx;
							break;
						}
					}
					colIdx++;
		...
		} else if(pstate->p_current_lambda->lambdakind == LAMBDA_SCALAR_CONTEXT)
		{
			String *name;
			int arg_idx;

			Assert(list_length(funcname) == 1 && IsA(linitial(funcname), String));
			name = linitial_node(String, funcname);

			/* Find index of prefix in argnames */
			arg_idx = list_index(pstate->p_current_lambda->argnames, name);
			...
			if (arg_idx >= 0)
			{
				...
				tmp_funcname = lcons(makeString(LAMBDA_PASS_TO_FUNC_DETAIL), funcname);
				l_ctx = pstate->p_lambdacontext;
				pg_attr = TupleDescAttr(RelationGetDescr(pstate->p_target_relation), l_ctx->targetattno);
				sig = LambdaSigGetSignature(pg_attr->atttypmod);

				attr = list_nth_node(LambdaSigType, sig->argtypes, arg_idx);
				rowIdx_of_match = arg_idx;
				matching_LambdaSigType = attr;

				Assert(matching_LambdaSigType != NULL && matching_LambdaSigType->is_function);	
			}
		}
	}

	/*
	 * If the previous checks failed, that means we are not in a lambda function,
	 * however, it is still possible, that the function we are calling right now is a lambda function 
	 * outside of the normal UDF context.
	 * 
	 * Because the code was written poorly and in two stages, the current implementation simply 
	 * prepends another magic prefix string, and func_get_detail knows to not throw a namespace
	 * not found error, and we have to handle possible errors/edgecases.
	 */
	possibleLambdaInNamespaceMatch = LookUpFuncNameAsLambda(pstate, funcname, location);
	if(possibleLambdaInNamespaceMatch != NULL)
	{
		tmp_funcname = lcons(makeString(LAMBDA_PASS_TO_FUNC_DETAIL_ON_MATCH), funcname);
	}

	/*
	 * Holes("_") request a partial application, only supported for a direct call
	 * of a column-stored(scalar-context) lambda. Reject the rest before coercion
	 * or lookup: a function-input call inside a lambda body(UDF path, not done
	 * yet), or any name that isnt a lambda column.
	 */
	if (nholes > 0)
	{
		if (matching_LambdaInput != NULL || matching_LambdaSigType != NULL)
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					 errmsg("partial application of a lambda function input is not supported"),
					 parser_errposition(pstate, location)));

		if (possibleLambdaInNamespaceMatch == NULL)
			ereport(ERROR,
					(errcode(ERRCODE_SYNTAX_ERROR),
					 errmsg("\"_\" is only allowed as an argument of a lambda call"),
					 parser_errposition(pstate, location)));
	}

	fdresult = func_get_detail(tmp_funcname, fargs, argnames, nargs,
							   actual_arg_types,
			...
			ereport(ERROR,
					(errcode(ERRCODE_UNDEFINED_FUNCTION),
					 ...
					 errhint("No procedure matches the given name and argument types. "
							 "You might need to add explicit type casts."),
					 parser_errposition(pstate, location)));
		} else if(!matching_LambdaInput && !matching_LambdaSigType && fdresult != FUNCDETAIL_LAMBDA) { //dont throw an error, if matching lambda was found
			ereport(ERROR,
				(errcode(ERRCODE_UNDEFINED_FUNCTION),
					errmsg("function %s does not exist",
						func_signature_string(funcname, nargs, argnames,
	...
	/* If we are working on a lambdaFuncAlias, do type checking and coercion */
	if(matching_LambdaInput)
	{
		int i = 0;

		declared_arg_types = palloc0(sizeof(Oid) * nargs);

		foreach(l, matching_LambdaInput->input_types)
		{
			LambdaInputParam *input = (LambdaInputParam *) lfirst(l);
			if(input->isAtomic)
				declared_arg_types[i++] = input->type;
			else 
				declared_arg_types[i++] = LAMBDAOID;
		}

		rettype = matching_LambdaInput->type;
	} else if (matching_LambdaSigType)
	{
		int i = 0;
		
		declared_arg_types = palloc0(sizeof(Oid) * nargs);

		foreach(l, ((LambdaSignature *) matching_LambdaSigType->signature)->argtypes)
		{
			LambdaSigType *input = (LambdaSigType *) lfirst(l);
			if(!input->is_function)
				declared_arg_types[i++] = input->typeoid;
			else
				declared_arg_types[i++] = LAMBDAOID;
		}

		rettype = ((LambdaSigType *) ((LambdaSignature *) matching_LambdaSigType->signature)->rettype)->typeoid;
	} else if (fdresult == FUNCDETAIL_LAMBDA)
	{
		Var				*match;
		LambdaSignature *sig_match;
		LambdaSigType	*ret;
		List			*coerced_args = NIL;
		Bitmapset	   *heldargs = NULL;
		ListCell		*lc_arg;
		ListCell		*lc_typ;
		int				 i = 0;
		CallLambdaExpr *cle;
		...
		if (possibleLambdaInNamespaceMatch == NULL ||
			!IsA(possibleLambdaInNamespaceMatch, Var))
			elog(ERROR, "FUNCDETAIL_LAMBDA without resolved lambda column");

		match = castNode(Var, possibleLambdaInNamespaceMatch);

		if (match->vartype != LAMBDAOID)
			elog(ERROR, "FUNCDETAIL_LAMBDA matched non-lambda column");

		if (match->vartypmod < 0)
			ereport(ERROR,
					(errcode(ERRCODE_DATATYPE_MISMATCH),
					errmsg("lambda column has no stored lambda signature"),
					parser_errposition(pstate, location)));

		sig_match = LambdaSigGetSignature(match->vartypmod);

		if (list_length(sig_match->argtypes) != nargs)
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					errmsg("lambda expects %d arguments, got %d",
							list_length(sig_match->argtypes), nargs),
					parser_errposition(pstate, location)));

		forboth(lc_arg, fargs, lc_typ, sig_match->argtypes)
		{
			Node		  *arg = (Node *) lfirst(lc_arg);
			LambdaSigType *argtyp = lfirst_node(LambdaSigType, lc_typ);

			/*
			 * held "_" carries no value: record its(0-based) position and drop it
			 * from args. Its declared type(argtyp) feeds the residual signature below.
			 */
			if (IsA(arg, HolePlaceholder))
			{
				heldargs = bms_add_member(heldargs, i);
				i++;
				continue;
			}

			if (argtyp->is_function)
			{
				if (actual_arg_types[i] != LAMBDAOID)
					ereport(ERROR,
							(errcode(ERRCODE_DATATYPE_MISMATCH),
							errmsg("lambda argument %d expects a lambda value", i + 1),
							parser_errposition(pstate, exprLocation(arg))));

				coerced_args = lappend(coerced_args, arg);
			}
			else
			{
				Node *coerced;
				...
				if (coerced == NULL)
					ereport(ERROR,
							(errcode(ERRCODE_DATATYPE_MISMATCH),
							errmsg("lambda argument %d cannot be coerced to type %s",
									i + 1,
									format_type_be(argtyp->typeoid)),
							parser_errposition(pstate, exprLocation(arg))));

				coerced_args = lappend(coerced_args, coerced);
			}

			i++;
		}

		ret = castNode(LambdaSigType, sig_match->rettype);

		/*
		 * Partial application: some arg was held with "_". Result is a lambda of
		 * signature "(held types) -> mother return type", intern it for a typmod
		 * and type the call as LAMBDAOID. Held positions live in cle->heldargs(args
		 * holds only the filled ones) for the executor to bake and re-emit. The
		 * residual is always a lambda, so fine even when ret->is_function.
		 */
		if (nholes > 0)
		{
			LambdaSignature *residual = makeNode(LambdaSignature);
			ListCell		*rc_arg;
			ListCell		*rc_typ;
			int32			 restypmod;

			forboth(rc_arg, fargs, rc_typ, sig_match->argtypes)
			{
				if (IsA((Node *) lfirst(rc_arg), HolePlaceholder))
					residual->argtypes = lappend(residual->argtypes,
												 lfirst_node(LambdaSigType, rc_typ));
			}
			residual->rettype = sig_match->rettype;
			residual->location = location;

			restypmod = LambdaSigCreateFromSignature(residual);

			cle = makeNode(CallLambdaExpr);
			cle->lambda_expr = (Expr *) match;
			cle->args = coerced_args;
			cle->heldargs = heldargs;
			cle->rettype = LAMBDAOID;
			cle->rettypmod = restypmod;
			cle->retcollid = get_typcollation(LAMBDAOID);
			cle->lambdatypmod = match->vartypmod;
			cle->location = location;

			/* Still a lambda call; a filled lambda value makes it higher order. */
			pstate->p_hasLambdaCall = true;
			for (int li = 0; li < nargs; li++)
			{
				if (actual_arg_types[li] == LAMBDAOID)
				{
					pstate->p_hasHigherOrderLambda = true;
					break;
				}
			}
		...
		if (ret->is_function)
		{
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					errmsg("currently, called lambda functions cannot return lambda functions")));
		}

		cle = makeNode(CallLambdaExpr);
		cle->lambda_expr = (Expr *) match;
		cle->args = coerced_args;
		cle->rettype = ret->typeoid;
		cle->rettypmod = -1;
		cle->retcollid = get_typcollation(ret->typeoid);
		cle->lambdatypmod = match->vartypmod;
		cle->location = location;

		/*
		 * Mark the query as containing a (UDF-free) lambda call so the planner's
		 * hoisting prepass knows to look. If any arg is itself a lambda value, this
		 * call is higher order and a hoisting candidate.
		 */
		pstate->p_hasLambdaCall = true;
		for (int li = 0; li < nargs; li++)
		{
	...
	/*
	 * A lambda input passed by name (e.g. b.f in b.g(...,b.f)) is a function
	 * value: an argless LambdaFuncAliasExpr typed as its return type. Where the
	 * target param is itself a function (LAMBDAOID), report LAMBDAOID so
	 * make_fn_arguments doesn't coerce the return type to "lambda".
	 */
	{
		ListCell   *lc_arg;
		int			i = 0;

		foreach(lc_arg, fargs)
		{
			Node *arg = (Node *) lfirst(lc_arg);

			if (declared_arg_types[i] == LAMBDAOID &&
				IsA(arg, LambdaFuncAliasExpr) &&
				((LambdaFuncAliasExpr *) arg)->args == NIL)
				actual_arg_types[i] = LAMBDAOID;
	...
	/*
	 * Nothing else to be done for Funccall to LambdaInput, except
	 * return a LambdaFuncAlias-node
	 */
	if(matching_LambdaInput)
	{
		LambdaFuncAliasExpr *lfa = makeNode(LambdaFuncAliasExpr);
		lfa->name = list_copy_deep(funcname);
		lfa->rettype = rettype;
		lfa->args = fargs;
		lfa->param_id = rowIdx_of_match + 1;
		lfa->fieldSelect_id = colIdx_of_match + 1;
		lfa->location = location;
		lfa->uniqueness = -1;
		return (Node *) lfa;
	} else if (matching_LambdaSigType)
	{
		//TODO: check for rewrite to CallLambdaExpr
		LambdaFuncAliasExpr *lfa = makeNode(LambdaFuncAliasExpr);
		lfa->name = list_copy_deep(funcname);
		lfa->rettype = rettype;
		lfa->args = fargs;
		lfa->param_id = 1;
		lfa->fieldSelect_id = rowIdx_of_match; //weirdness due to how accessing the passed vars works later on
		lfa->location = location;
		lfa->uniqueness = -1;
		return (Node *) lfa;
	}
	...
	/* 
	* Collect a TupleDesc for each Sublink, to help finding Type mismatches later in ColumnRefHooks
	* One TupleDesc per Sublink, so either one, or n, where n is the number of Lambdas aswell
	* ltiList will be passed and used to/in LambdaColumnRefHook
	*/
	foreach(l, fargs)
	{
		...
		if (IsA(arg, SubLink))
		{
			...
			TupleDesc tupdesc = CreateTemplateTupleDesc(list_length(qtree->targetList));
			...
			foreach(n, qtree->targetList)
			{
				...
				/*
				 * Strip the display-only "(id:N)" suffix (lambdaDecorateTargetName)
				 * from lambda columns: LambdaColumnRefHook matches a body's "b.col"
				 * against this tupdesc by the real column name.
				 */
				TupleDescInitEntry(tupdesc, (AttrNumber) (++idx),
						lambdaUndecorateName((Node *) tent->expr, tent->resname),
	...
	/*
	* If Sublinks where found, it means we might have lambdas and sublinks, if yes, 
	* recursively transform all lambda-expr bodies, and type check them
	* 
	* Also checks for duplicate variable names, etc.
	*/
	if (list_length(ltiList) > 0)
	{
		...
		foreach(l, fargs)
		{
			...
			if (IsA(arg, LambdaExpr)) {
				...
				lambda->argTypesTupDesc = NIL;
				pstate->p_current_ltis = ltiList;
				pstate->p_current_lambda = lambda;

				foreach(lambdaParamName, lambda->argnames)
				{
					...
					lambda->argTypesTupDesc = lappend(lambda->argTypesTupDesc,
						lfirst(subtableInfo));

					if (argCounter < list_length(ltiList) - 1)
						subtableInfo = lnext(ltiList, subtableInfo);
...
/*
 * LookUpFuncNameAsLambda
 * 
 * This helper function checks, wether the current func call could be
 * a lambda function being used in the current query namespace.
 * This is only allowed, if no schema collision exists and both the 
 * table/relation name and column alias/name are correct. In addition,
 * checks on type correctness can also be done here later on.
 */
static Node * 
LookUpFuncNameAsLambda(ParseState *pstate, List *funcname, ParseLoc location)
{
	char			   *refname;
	char			   *colname;
	...
	refname = strVal(linitial(funcname));
	colname = strVal(lsecond(funcname));

	/* first part = visible relation alias in the current namespace(respects nesting via sublevels_up) */
	...
	/* not column-visible -> not a lambda receiver */
	if (!nsitem->p_cols_visible)
		return NULL;

	/*
	 * second part = a column of that nsitem. scanNSItemForColumn() hands back a Var
	 * already filled with type, typmod, collation, varno, varattno, varlevelsup, etc.
	 */
	var = (Var *) scanNSItemForColumn(pstate,
							  nsitem,
							  sublevels_up,
							  colname,
							  location);
	...
	if (var->vartype != LAMBDAOID)
		return NULL;

	if (var->vartypmod < 0)
		ereport(ERROR,
				(errcode(ERRCODE_DATATYPE_MISMATCH),
				 errmsg("lambda column \"%s.%s\" has no stored lambda signature",
						refname, colname),
				 parser_errposition(pstate, location)));
				...
				List **argdefaults) /* optional return value */
{
	...
	bool in_lambda_ctx = false;
	bool lambda_column_matched = false;
	...
	*true_typeids = NULL;
	...
	/* Check if we are in a lambda-function */
	if(funcname 
		&& list_length(funcname) >=1 
		&& IsA(linitial(funcname), String) 
		&& pg_strcasecmp(LAMBDA_PASS_TO_FUNC_DETAIL, (linitial_node(String, funcname)->sval)) == 0)
	{
		in_lambda_ctx = true;
		funcname = list_delete_first(funcname);
	} else if(funcname 
		&& list_length(funcname) >= 1
		&& IsA(linitial(funcname), String) 
		&& pg_strcasecmp(LAMBDA_PASS_TO_FUNC_DETAIL_ON_MATCH, (linitial_node(String, funcname)->sval)) == 0)
	{
		in_lambda_ctx = true;
		funcname = list_delete_first(funcname);
		lambda_column_matched = true;
	}

	/* Get list of possible candidates from namespace search */
	raw_candidates = FuncnameGetCandidates(funcname, nargs, fargnames,
										   expand_variadic, expand_defaults,
										   include_out_arguments, in_lambda_ctx);

	/* If we are in a lambda context and a collision was found, throw error */
	if(in_lambda_ctx && raw_candidates != NULL)
	{
		ereport(ERROR,
        (errcode(ERRCODE_AMBIGUOUS_FUNCTION),
         errmsg("[func_get_detail] lambda function name \"%s\" collides with existing function namespace",
                NameListToString(funcname))));
	} else if(in_lambda_ctx && raw_candidates == NULL && lambda_column_matched)
	{
		/*
		 * If no collision was detected and the func call is confirmed to be a 
		 * lambda column, we proceed to treat the call as such.
		 */
		return FUNCDETAIL_LAMBDA;
	}
