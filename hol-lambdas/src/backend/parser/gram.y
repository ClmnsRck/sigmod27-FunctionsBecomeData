...
static Node *makeLambdaInputParam(char *colname, Node *typeNode, int idx, ParseLoc loc);
static Node *makeLambdaExpr(List *args, List *types, Node *expr, LambdaKind kind, ParseLoc location);
static Node *makeLambdaSignature(List *argtypes, Node *rettype, int location);
static Node *makeLambdaSigScalarType(TypeName *typename, int location);
static Node *makeLambdaSigFunctionType(Node *signature, int location);
...
%type <list>		lambda_row_input_list 
%type <list>		lambda_row_input
%type <list>		lambda_parameters 
%type <list>		lambda_parameter_type_list
%type <node>		lambda_parameter 
%type <node>		lambda_parameter_type

%type <node>		lambda_scalar_type
%type <list>		lambda_scalar_type_list
%type <node>		lambda_type_signature
%type <node>		lambda_typename

%type <list>		lambda_name_list
...
%token <keyword> ABORT_P ABSENT ABSOLUTE_P ACCESS ACTION ADD_P ADMIN AFTER
	...
	HANDLER HAVING HEADER_P HOLD HOLE_P HOUR_P
...
/* lril is the supertype, lri is one of the row-aliases, lp's is the list of individual inputs, lp is a single column in a row, lpt is the type of a single column  */
lambda_row_input_list:	lambda_row_input
						{
							List *names;
							List *parameters;

							names = list_make1(linitial($1));
							parameters = list_make1(lsecond($1));

							$$ = list_make2(names, parameters);
						}
					| lambda_row_input_list ',' lambda_row_input
						{
							List *names;
							...
							names = (List *) linitial($1);
							parameters = (List *) lsecond($1);

							names = lappend(names, linitial($3));
							parameters = lappend(parameters, lsecond($3));

							$$ = list_make2(names, parameters);
						}
		;

lambda_row_input:	ColId ':' '[' lambda_parameters ']' 
						{ 
							List *pair;
							pair = list_make2(makeString(pstrdup($1)), $4);
							$$ = pair;
						}
		;
...
lambda_parameter:	ColId ':' lambda_parameter_type
						{
							LambdaInputParam *param; 
							param = (LambdaInputParam *) $3;
							param->name = list_make1(makeString(pstrdup($1)));
							$$ = (Node *) param;
						}
		;

lambda_parameter_type: Typename
						{
							LambdaInputParam *param;
							param = (LambdaInputParam *) makeLambdaInputParam(NULL, (Node *) $1, -1, @1);
							param->isAtomic = true;
							param->input_types = NIL;
							$$ = (Node *) param;
						}
						| '(' lambda_parameter_type_list Op Typename ')'
						{
							if(strcmp($3, "->") != 0) 
								ereport(ERROR, (errcode(ERRCODE_SYNTAX_ERROR)), errmsg("expected -> in lambda type!"));
							
							LambdaInputParam *param;
							param = (LambdaInputParam *) makeLambdaInputParam(NULL, (Node *) $4, -1, @1);
							param->isAtomic = false;
							param->input_types = $2;
							$$ = (Node *) param;
						}
		;

lambda_parameter_type_list: lambda_parameter_type
						{
							$$ = list_make1($1);
						} 
						| lambda_parameter_type_list ',' lambda_parameter_type
...
/*lambda_type/lambda_scalar_signature are the lambda signatures found only int DDL column access language (CREATE TABLE, etc)*/
lambda_type_signature:
        lambda_scalar_type_list Op lambda_scalar_type
            {
				if(strcmp($2, "->") != 0) 
					ereport(ERROR, (errcode(ERRCODE_SYNTAX_ERROR)), errmsg("expected -> in lambda type!"));

                $$ = makeLambdaSignature($1, $3, @1);
            }
;

lambda_scalar_type_list:
        lambda_scalar_type
            {
                $$ = list_make1($1);
            }
        | lambda_scalar_type_list ',' lambda_scalar_type
            {
                $$ = lappend($1, $3);
            }
;
...
lambda_scalar_type:
        Typename
            {
                /* atomic/scalar type, only an ordinary TypeName(no names, no row aliases) */
                $$ = (Node *) makeLambdaSigScalarType($1, @1);
            }
        | '(' lambda_type_signature ')'
            {
                /* nested function type, e.g. (int4, text -> bool) */
                $$ = (Node *) makeLambdaSigFunctionType($2, @1);
            }
;

/*
 * Untyped scalar lambda params, for lambda values like lambda(x, y)(x + y).
 * No row aliases, no types, no return signature.
 */
lambda_name_list:
        ColId
            {
        ...
        | lambda_name_list ',' ColId
            {
                $$ = lappend($1, makeString(pstrdup($3)));
            }
;
...
lambda_typename:
			LAMBDA '(' lambda_type_signature ')'
				{
					TypeName *n;

					n = makeTypeName("lambda");
					n->location = @1;
					n->lambda_signature = $3;
...
a_expr:		c_expr									{ $$ = $1; }
		 ...
		 */
			...
			*/
			...
			/*
			 * Lambda functions occur in two main versions:
			 * They are either meant as the top-level lambda function in a UDF function, i.e.
			 * the function directly in the function call in SQL, or a lambda function definition 
			 * literally anywhere else, i.e. in a insert/create statement(those are split again, as
			 * a create statement does not need names/aliases, whilst the insert statement has no need 
			 * for types)
			 */
			| LAMBDA_SIGN '(' lambda_row_input_list ')' a_expr
				{					
					List *names = (List *) linitial($3);
					List *types = (List *) lsecond($3);
					$$ = makeLambdaExpr(names, types, $5, LAMBDA_ROW_CONTEXT, @1);
				}

			| LAMBDA '(' lambda_row_input_list ')' a_expr
				{					
					List *names = (List *) linitial($3);
					List *types = (List *) lsecond($3);
					$$ = makeLambdaExpr(names, types, $5, LAMBDA_ROW_CONTEXT, @1);
			...
			| LAMBDA_SIGN '(' lambda_name_list ')' a_expr
				{
					List *names = $3;
					List *types = NIL;
					$$ = makeLambdaExpr(names, types, $5, LAMBDA_SCALAR_CONTEXT, @1);
				}
			
			| LAMBDA '(' lambda_name_list ')' a_expr
				{
					List *names = $3;
					List *types = NIL;
					$$ = makeLambdaExpr(names, types, $5, LAMBDA_SCALAR_CONTEXT, @1);
...
c_expr:		columnref								{ $$ = $1; }
			...
			| HOLE_P
				{
					HolePlaceholder *h = makeNode(HolePlaceholder);
...
static Node *
makeLambdaInputParam(char *colname, Node *typeNode, int idx, ParseLoc loc)
{
	LambdaInputParam *param = makeNode(LambdaInputParam);
	if(colname)
		param->name = list_make1(makeString(pstrdup(colname)));
	else
		param->name = NIL;
	param->location = loc;
	param->idx = idx;
	...
	{
		Oid type_oid = LookupTypeNameOid(NULL, (TypeName *) typeNode, false);
		if (!OidIsValid(type_oid))
		{
			ereport(ERROR,
			        (errcode(ERRCODE_UNDEFINED_OBJECT),
			         errmsg("could not resolve type for lambda parameter %s",
			                colname ? colname : "<anonymous>")));
		}
	...
	else
	{
		ereport(ERROR,
		        (errcode(ERRCODE_SYNTAX_ERROR),
		         errmsg("invalid lambda parameter type"),
		         errdetail("Lambda parameter \"%s\" does not have a valid type expression.", 
		                   colname ? colname : "<anonymous>")));
	}
...
static Node *
makeLambdaExpr(List *args, List *types, Node *expr, LambdaKind kind, ParseLoc location)
{
...
static Node *
makeLambdaSignature(List *argtypes, Node *rettype, int location)
{
    LambdaSignature *n = makeNode(LambdaSignature);

    n->argtypes = argtypes;
    n->rettype = rettype; //LambdaSigType
    n->location = location;
...
static Node *
makeLambdaSigScalarType(TypeName *typename, int location)
{
    LambdaSigType *n = makeNode(LambdaSigType);

	if (IsA(typename, TypeName))
	{
		Oid type_oid = LookupTypeNameOid(NULL, typename, false);
		if (!OidIsValid(type_oid))
		{
			ereport(ERROR,
			        (errcode(ERRCODE_UNDEFINED_OBJECT),
			         errmsg("could not resolve types for lambda parameter")));
		}
	...
	else
	{
		ereport(ERROR,
		        (errcode(ERRCODE_SYNTAX_ERROR),
		         errmsg("invalid lambda parameter type"),
		         errdetail("Lambda parameters do not have a valid type expression.")));
	}
...
static Node *
makeLambdaSigFunctionType(Node *signature, int location)
{
    LambdaSigType *n = makeNode(LambdaSigType);

    n->is_function = true;
    n->typeoid = InvalidOid;
