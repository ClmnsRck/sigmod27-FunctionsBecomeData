...
/* 
	Helper function to create a Const representation of a LambdaExpr.
	LambdaFunctions outside of UDFS will always be cast/converted into SerialLambdaExprs to ensure TOAST compliance and storability.
	Used in the Backend to mark Lambda expressions/functions that can live outside of UDFs.
	These Lambda functions should not be executed or parsed unless you know what you are doing.
	To use them, refer to the use of PG_LAMBDA_<> Makros and their usage in src/ext/<...>.c and the functions defined/used there.
*/
Const *
makeLambdaConst(ParseState *pstate, LambdaExpr *expr)
{
	Const	   		   *con;
	Datum				val;
	Oid					typeid;
	...
	sl = lambdaExprToSerialLambda(expr);

	typeid = LAMBDAOID;
	typelen = -1;
	val = PointerGetDatum(sl);
	typebyval = false;
