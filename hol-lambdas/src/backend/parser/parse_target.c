					 ...
					 bool resjunk)
{
	...
	if (colname == NULL && !resjunk)
	{
		...
		generated_colname = true;
	}

	/*
	 * Keep explicit aliases as-is. Decorating generated names is fine for display,
	 * but rewriting user-given aliases leaks signatures into semantic column
	 * names(e.g. CTAS output columns).
	 */
	if (!resjunk && generated_colname && exprType(expr) == LAMBDAOID)
		colname = lambdaDecorateTargetName((Node *) expr, colname);
						...
						ParseExprKind exprKind, bool allowDefault)
{
	...
	foreach(lc, exprlist)
	{
		...
		} else if (IsA(e, LambdaExpr))
		{
			LambdaContext *l_ctx = palloc0(sizeof(LambdaContext));

			l_ctx->targetattno = ctr;
			pstate->p_lambdacontext = l_ctx;
		}
