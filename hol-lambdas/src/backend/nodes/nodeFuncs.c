...
exprType(const Node *expr)
{
	...
	switch (nodeTag(expr))
	{
		...
		case T_LambdaFuncAliasExpr:
			type = ((const LambdaFuncAliasExpr *) expr)->rettype;
			break;
		case T_LambdaInputParam:
			type = ((const LambdaInputParam *) expr)->type;
			break;
		case T_LambdaSignature:
			type = LAMBDAOID;
			break;
		case T_LambdaSigType:
			{
				const LambdaSigType *typ = (const LambdaSigType *) expr;

				type = typ->is_function ? LAMBDAOID : typ->typeoid;
			}
			break;
		case T_CallLambdaExpr:
			type = ((const CallLambdaExpr *) expr)->rettype;
			break;
		case T_Param:
...
exprTypmod(const Node *expr)
{
	...
	switch (nodeTag(expr))
	{
		...
		case T_LambdaExpr:
			return ((const LambdaExpr *) expr)->lambdatypmod;
		case T_CallLambdaExpr:
			return ((const CallLambdaExpr *) expr)->rettypmod;
		case T_LambdaInputParam:
		case T_LambdaFuncAliasExpr:
			return -1;
		case T_Param:
...
exprCollation(const Node *expr)
{
	...
	switch (nodeTag(expr))
	{
		...
		case T_HolePlaceholder:
			...
			break;
		case T_CallLambdaExpr:
			coll = ((const CallLambdaExpr *) expr)->retcollid;
			break;
		case T_Param:
...
exprSetCollation(Node *expr, Oid collation)
{
	switch (nodeTag(expr))
	{
		...
		case T_LambdaSigType:
			...
			break;
		case T_CallLambdaExpr:
			((CallLambdaExpr *) expr)->retcollid = collation;
			break;
		case T_Param:
...
exprLocation(const Node *expr)
{
	...
	switch (nodeTag(expr))
	{
		...
		case T_LambdaExpr:
			...
			break;
		case T_LambdaFuncAliasExpr:
			loc = ((const LambdaFuncAliasExpr *) expr)->location;
			break;
		case T_LambdaInputParam:
			loc = ((const LambdaInputParam *) expr)->location;
			break;
		case T_LambdaSignature:
			loc = ((const LambdaSignature *) expr)->location;
			break;
		case T_LambdaSigType:
			loc = ((const LambdaSigType *) expr)->location;
			break;
		case T_CallLambdaExpr:
			loc = ((const CallLambdaExpr *) expr)->location;
			break;
		case T_Param:
		...
		case T_SetToDefault:
			...
			break;
		case T_HolePlaceholder:
			loc = ((const HolePlaceholder *) expr)->location;
			break;
		case T_TargetEntry:
			/* just use argument's location */
...
#define LIST_WALK(l) expression_tree_walker_impl((Node *) (l), walker, context)
	...
	switch (nodeTag(node))
	{
		...
		case T_LambdaFuncAliasExpr:
			{
				LambdaFuncAliasExpr *l_alias = (LambdaFuncAliasExpr *) node;

				if (LIST_WALK(l_alias->args))
					return true;
			}
			break;
		case T_LambdaSignature:
			{
				LambdaSignature *sig = (LambdaSignature *) node;

				if(LIST_WALK(sig->argtypes))
					return true;
				if(WALK(sig->rettype))
					return true;
			}
		...
		case T_LambdaSigType:
			{
				LambdaSigType* typ = (LambdaSigType *) node;

				if(WALK(typ->signature))
					return true;
			}
			break;
		case T_CallLambdaExpr:
			{
				...
				if(WALK(cle->lambda_expr))
					return true;
				if(LIST_WALK(cle->args))
					return true;
			}
			break;
		...
		case T_LambdaExpr:
			{
				...
				newnode->argnames = list_copy_deep(le->argnames);
				newnode->argtypes = list_copy_deep(le->argtypes);
				newnode->argTypesTupDesc = list_copy(le->argTypesTupDesc);
				return (Node *) newnode;
			}
		case T_LambdaFuncAliasExpr:
			{
				LambdaFuncAliasExpr *la = (LambdaFuncAliasExpr *) node;
				LambdaFuncAliasExpr *newnode;

				FLATCOPY(newnode, la, LambdaFuncAliasExpr);
				MUTATE(newnode->args, la->args, List *);
				newnode->name = list_copy_deep(la->name);
				return (Node *) newnode;
			}
		case T_LambdaInputParam:
			{
				LambdaInputParam *lip = (LambdaInputParam *) node;
				LambdaInputParam *newnode;

				FLATCOPY(newnode, lip, LambdaInputParam);
				newnode->input_types = list_copy_deep(lip->input_types);
				newnode->name = list_copy_deep(lip->name);
				return (Node *) newnode;
			}
		case T_LambdaSignature:
			{
				LambdaSignature *sig = (LambdaSignature *) node;
				LambdaSignature *newnode;

				FLATCOPY(newnode, sig, LambdaSignature);
				MUTATE(newnode->argtypes, sig->argtypes, List *);
				MUTATE(newnode->rettype, sig->rettype, Node *);
				return (Node *) newnode;
			}
		case T_LambdaSigType:
			{
				LambdaSigType *typ = (LambdaSigType *) node;
				LambdaSigType *newnode;

				FLATCOPY(newnode, typ, LambdaSigType);
				MUTATE(newnode->signature, typ->signature, Node *);
				return (Node *) newnode;
			}
		case T_CallLambdaExpr:
			{
				CallLambdaExpr *cle = (CallLambdaExpr *) node;
				CallLambdaExpr *newnode;

				FLATCOPY(newnode, cle, CallLambdaExpr);
				MUTATE(newnode->lambda_expr, cle->lambda_expr, Expr *);
				MUTATE(newnode->args, cle->args, List *);
				return (Node *) newnode;
			}
		case T_WithCheckOption:
			{
								...
								void *context)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_LambdaFuncAliasExpr:
			{
				LambdaFuncAliasExpr *le = (LambdaFuncAliasExpr *) node;
