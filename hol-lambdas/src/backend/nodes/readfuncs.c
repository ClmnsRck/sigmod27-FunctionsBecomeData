...
static LambdaExpr *
_readLambdaExpr(void) 
{
	READ_LOCALS(LambdaExpr);

	READ_NODE_FIELD(argnames);
	READ_NODE_FIELD(argtypes);
	READ_BOOL_FIELD(is_strict);
	READ_BOOL_FIELD(l3_safe);
	READ_ENUM_FIELD(lambdakind, LambdaKind);
	READ_OID_FIELD(rettype);
	READ_INT_FIELD(rettypmod);
	READ_INT_FIELD(lambdatypmod);
	READ_NODE_FIELD(expr);

	READ_DONE();
}

static LambdaInputParam *
_readLambdaInputParam(void) 
{
	READ_LOCALS(LambdaInputParam);

	READ_NODE_FIELD(name);
	READ_OID_FIELD(type);
	READ_BOOL_FIELD(isAtomic);
	READ_NODE_FIELD(input_types);
	READ_INT_FIELD(idx);
	READ_LOCATION_FIELD(location);
