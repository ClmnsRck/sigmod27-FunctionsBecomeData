...
" unless $struct_no_equal;
	...
	foreach my $f (@{ $node_type_info{$n}->{fields} })
	{
		...
		{
			#Copy 'TupleDesc' if we are inside lambda functions, hope this breaks nothing
			print $cff "\tCOPY_SCALAR_FIELD($f);\n" unless $copy_ignore;
		}
		else
		{
...
! unless $no_read;
		...
		{
			#Ignore 'TupleDesc' in read and write, as any other instance would have already died, and this is only to skip them in
		}
		else
		{
			die
