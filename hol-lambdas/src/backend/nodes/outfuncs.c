...
_outLambdaExpr(StringInfo str, const LambdaExpr *node) 
{
	...
	WRITE_NODE_FIELD(argnames);
	WRITE_NODE_FIELD(argtypes);
	WRITE_BOOL_FIELD(is_strict);
	WRITE_BOOL_FIELD(l3_safe);
	WRITE_ENUM_FIELD(lambdakind, LambdaKind);
	WRITE_OID_FIELD(rettype);
	WRITE_INT_FIELD(rettypmod);
	WRITE_INT_FIELD(lambdatypmod);
	WRITE_NODE_FIELD(expr);
}

static void
_outLambdaInputParam(StringInfo str, const LambdaInputParam *node) 
{
	WRITE_NODE_TYPE("LAMBDAINPUTPARAM");
	WRITE_NODE_FIELD(name);
	WRITE_OID_FIELD(type);
	WRITE_BOOL_FIELD(isAtomic);
	WRITE_NODE_FIELD(input_types);
	WRITE_INT_FIELD(idx);
	WRITE_LOCATION_FIELD(location);
}
