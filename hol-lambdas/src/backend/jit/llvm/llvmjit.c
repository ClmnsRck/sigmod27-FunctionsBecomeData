...
llvm_release_context(JitContext *context)
{
	...
	if(llvm_jit_context->moduleCopy)
	{
		LLVMDisposeModule(llvm_jit_context->moduleCopy);
		llvm_jit_context->moduleCopy = NULL;
	}
...
/*
 * Closes the temporary JIT context and switches back to the main JIT context.
 */
void
llvm_close_tmp_context(EState *state)
{	
	if(!state || !state->es_jit)
		return;
	...
	if(state->es_jit_tmp)
		llvm_release_context(state->es_jit_tmp);
	state->es_jit_tmp = NULL;
	llvm_leave_fatal_on_oom();
}
...
llvm_create_types(void)
{
	...
	StructArrayType = llvm_pg_var_type("StructArrayType");
