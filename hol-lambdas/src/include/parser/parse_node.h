...
typedef struct LambdaContext LambdaContext;
...
typedef enum ParseExprKind
{
	...
	EXPR_KIND_LAMBDA_EXPRESSION,/* lambda expressions */
...
struct ParseState
{
	...
	LambdaExpr 	*p_current_lambda; /* currently parsed lambda */
	List 		*p_current_ltis; 	/* List of lambdatable infos for tablefunc currently being parsed */
	int			p_idOfCurrentLambda;/* the lambda, currently being checked for duck-matching its inputs */
	LambdaContext *p_lambdacontext; /* holds info to parse lambda functions outside of udfs */
	...
	bool		p_hasLambdaCall;
	bool		p_hasHigherOrderLambda;
...
/*
 * Holds the target column for InsertStatements, etc.
 *
 * If we handle a insert statement, lambda functions have no idea about the target column
 * and cannot check, wether the signature matches and produces legal expressions or makes use of
 * illegal operations, sntax, etc.
 */
struct LambdaContext {
	int32 targettypmod;			/* holds the signature-id */
	AttrNumber targetattno;		/* holds the column to which we map */
};
...
extern Const *makeLambdaConst(ParseState *pstate, LambdaExpr *expr);
