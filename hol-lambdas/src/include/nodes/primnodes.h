...
typedef struct Param
{
	...
	/* token location, or -1 if unknown */
	ParseLoc	location;
	/* indicates wether it is used in lambda expr */
	bool lambda pg_node_attr(query_jumble_ignore, equal_ignore);
} Param;
...
/*
 * LambdaSignature/LambdaSigType - nameless signature of lambda columns
 * 
 * This only appears in CREATE TABLE statements where lambda functions need to define their 
 * signature/shape to ensure proper typing. The type is only needed for parsing, as the
 * finished signature is stored in a fully-independent pg_catalog(pg_lambdasig).
 */
typedef struct LambdaSignature
{
    Expr        xpr;
    List       *argtypes;     /* List of LambdaSigType */
    Node       *rettype;      /* LambdaSigType */
    ParseLoc    location;
} LambdaSignature;

typedef struct LambdaSigType
{
    Expr        xpr;
    bool        is_function;
    Oid   		typeoid;     /* if scalar, else INVALID(not sure yet TODO) */
    Node       *signature;    /* if nested function */
    ParseLoc    location;
} LambdaSigType;

/*
 * Distinguishes Lambda functions
 *
 * Two different versions:
 * - LAMBDA_ROW_CONTEXT: Lambda functions meant to be used in UDFs, where the main/
 *                       only input will be full tuple rows
 * - LAMBDA_SCALAR_CONTEXT: Lambda functions that functions normally outside of UDF
 *                          contexts and can be considered a function call
 *                          like any other cataloged PG function
 * 
 * This change allows for backwards compatability between UDF called lambda functions
 * and "normal" lambda functions that are used as scalar functions in normal SQL
 * 
 */
typedef enum LambdaKind
{
    LAMBDA_ROW_CONTEXT,    /* UDF-only */
    LAMBDA_SCALAR_CONTEXT  /* column-stored lambdas legal anywhere */
} LambdaKind;

/*
 * LambdaInputParam - a column of a lambda-input-row.
 *
 * These nodes are a representation of a single column of an input-row.
 * Meaning in: lambda(a:{x:float, y:float}, b:{f:(...), g:(...)})(<expr_body>)
 * they represent the x/y/f/g. The Node holds its name, index in the row, retype
 * and wether its a function or an atomic, and if it is a function, its inputs and outputs.
 * 
 * Currently, functions-as-inputs are only allowed to return an atomic value, to reduce 
 * system complexity.
 */
typedef struct LambdaInputParam
{
	pg_node_attr(no_query_jumble, custom_read_write)

	Expr 		xpr; 
	/* holds the name of the column */
	List    	*name;
	/* type of the parameter, or its return type, if its a function */
	Oid 		type;
	/* wether the parameter is actually a function-input */
	bool 		isAtomic;
	/* the types of the inputs, NULL if not a func, 
		List*<LambdaInputParam> if func(arbitrarily recursive) */
	List    	*input_types;
	/* the index in the inputs of the lambdaExpr, just a convienence look-up */
	int 		idx;
	/* location information for the parser */
	ParseLoc 	location pg_node_attr(equal_ignore); 
} LambdaInputParam;

/*
 * LambdaFuncAlias - a part of a lambda-expression.
 *
 * These nodes signify the call to an undefined Lambda function during execution.
 * They are required, because the planner/parser dont know which lambda expression 
 * tree to inject yet, but they need information about it, such as the return type,
 * its relative position in the inputs of the calling lambda(i.e. the current 
 * lambda being parsed). Therefore this node is introduced to keep track of funccalls,
 * that are present in inputs, but do not exist in the PG_catalog.
 * 
 * These Nodes can only exist in lambda-expression bodies, and are only used to mark 
 * the injection point of called lambdas generically during planning. 
 * 
 * NOTICE: These nodes are removed during injection. Should a LambdaFuncAlias remain
 * afterwards, a fatal error was encountered adn the Execution must be stopped.
 */
typedef struct LambdaFuncAliasExpr
{
	pg_node_attr(no_query_jumble)

	Expr 	xpr; 
	/* holds the name of the function call */
	List    *name;
	/* return type of the result */
	Oid 	rettype;
	/* stores the Param_ID and the FieldSelect_ID */
	int32 	param_id;
	int32 	fieldSelect_id;
	/* stores wether a LFA is unique in the current lambda */
	int32 	uniqueness pg_node_attr(equal_ignore); 
	/* 1-to-1 map of the FuncCall-node arg-expressions transformed into this node */
	List 	*args; 
	/* Praser-Error information */
	ParseLoc 	location pg_node_attr(equal_ignore); 
} LambdaFuncAliasExpr;
 ...
 * table function.
 * 
 * As this is the primary implementation root of lambda functions, this
 * explanation will go here:
 * Lambda functions occur in two forms currently:
 * - Lambda functions in a UDF context are passed directly two a UDF/table function,
 *   their inputs being stored as <rowalias>.<columnalias> meaning a two tiered storage
 *   system (LAMBDA_ROW_CONTEXT)
 *   These lambda functions will fail anywhere where standard function lookups are 
 *   supposed to occur, as those expect arguments in a single-tiered look-up
 * - Lambda functions that mimic normal functions/function call to standard,
 *   cataloged pg functions, e.g. add, mult, etc.
 *   These functions can be executed anywhere, as long as their inputs are supplied 
 *   correctly, like any other function. (LAMBDA_SCALAR_CONTEXT)
 */
typedef struct LambdaExpr
{
	...
	/* Stores the kind of lambdaexpression, wether its a top-level UDF lambda, or not */
	LambdaKind  lambdakind;

	/* Argument names (List of String or Value nodes, one per input) */
	List	   *argnames;

	/* Argument types (List of List of LambdaInputParam) */
	List	   *argtypes;

	/* TupleDescriptor of the Input, needed for executor routines (runtime-only) */
	List 	   *argTypesTupDesc pg_node_attr(equal_ignore, copy_as_scalar);

	/* Expression body (a_expr → Expr) */
	Expr	   *expr;

	/* Type/typmod of the BODY: the value produced when the lambda is CALLED. */
	Oid         rettype;
	int         rettypmod;

	/*
	 * Signature typmod of the lambda value itself(exprType == LAMBDAOID), keys
	 * pg_lambdasig (cf. CallLambdaExpr.lambdatypmod). Reported by exprTypmod(), -1
	 * if unresolved. Derived key, ignored for equality.
	 */
	int32       lambdatypmod pg_node_attr(equal_ignore);

	/* Strictness flag: if true, lambda returns NULL if any arg is NULL */
	bool        is_strict;

	/*
	 * L3 optimizability verdict(derived). True only if every op/func node in `expr`
	 * calls a function with a hand-written L3 impl(see the L3 whitelist in
	 * utils/lambda.h), every leaf is L3-safe(Const or a PARAM_EXEC Param), and every
	 * nested lambda call(LFA/CallLambdaExpr) has only L3-safe arg expressions. A
	 * nested call's callee body is a runtime binding, so its own L3-safety isnt
	 * decided here - its AND-ed in at injection time, where every lambda in a combo
	 * must be l3_safe. Computed once at parse analysis by lambda_expr_is_l3_safe().
	 *
	 * Defaults to false, so an un-analyzed lambda(or one using anything off the
	 * whitelist) is never mistaken for optimizable - the fast/unsafe build path may
	 * only run when this is explicitly true. Pure cache of a property fully
	 * determined by `expr`, so ignored for equality.
	 */
	bool        l3_safe pg_node_attr(equal_ignore);

	/* Execution-time state (not serialized) */
	/* Pointers to structs needed for exectuion */
	Node       *exprstate pg_node_attr(equal_ignore, copy_as_scalar);
	Node       *econtext  pg_node_attr(equal_ignore, copy_as_scalar);
	Node  	   *parentPlan pg_node_attr(equal_ignore, copy_as_scalar);
 ...
 * CallLambdaExpr
 *
 * A CallLambdaExpr represents the direct usage of a Lambda function in 
 * SQL outside of a UDF made to handle LambdaFunctions. The Parser creates
 * these nodes, when a function call could not be resolved to a schema-qualified
 * function or cataloged function.
 * Should a table in the query namespace exist, that matches the first part of the
 * function call, and said table contains a column of type LambdaExpr, whose arguments
 * match with the encountered node, it will be resolved to a CallLambdaExpr as a fallback,
 * to ensure mostly undisturbed Postgres semantics.
 * 
 *  Todo: explanation of fields
 */
typedef struct CallLambdaExpr 
{
	pg_node_attr(no_query_jumble)

	Expr 		xpr;

	/* The LambdaExpr, that will be evaluated */
	Expr 		*lambda_expr;

	/* The provided arguments (filled positions only; holes are not stored here) */
	List 		*args;

	/*
	 * Partial application: 0-based mother arg positions held with "_". NULL for an
	 * ordinary(fully applied) call. When set, the call evaluates to a residual
	 * lambda value(rettype == LAMBDAOID) instead of a scalar, and args holds only
	 * the filled positions, in mother-argument order.
	 */
	Bitmapset	*heldargs;

	/* Resolved information/helper meta-data */
	Oid 		rettype;
	int32 		rettypmod;
	Oid 		retcollid;

	/* The Signature struct holding information usefule for debugging */
	int32 		lambdatypmod;
...
typedef struct FieldSelect
{
	...
	/* TupleDesc of rowtype, only needed inside of LambdaFuncs */
	TupleDesc 	lambda 				pg_node_attr(query_jumble_ignore, equal_ignore);
	/* void pointer to the explicit name, just for the conversion from lambda to serialized */ 
	char 	   *fieldName_payload	pg_node_attr(query_jumble_ignore, equal_ignore);
} FieldSelect;
