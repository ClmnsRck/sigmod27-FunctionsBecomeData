...
typedef struct Query
{
	...
	/* has a CallLambdaExpr (UDF-free lambda call) somewhere in the query */
	bool		hasLambdaCall pg_node_attr(query_jumble_ignore);
	/* at least one such lambda call takes a lambda-valued argument (higher order) */
	bool		hasHigherOrderLambda pg_node_attr(query_jumble_ignore);
...
struct TypeName;  // forward declare
 ...
 * 
 * If lambda_* is set, it means these typenames are inside a lambda column
 * definition/declaration. They will be turned into a pg_lambdasig entry
...
struct TypeName
{
	...
	Node 	   *lambda_signature;	/* lambda input signature parts, or NULL */
...
/*
 * HolePlaceholder - the "_" hole marker for lambda partial application.
 *
 * Produced by the grammar for the reserved word "_"(the HOLE_P token). Raw-parse-
 * tree-only node, marks a held(unbound) arg position in a lambda call so the call
 * can later become a partial application. No payload beyond its location. "_" is
 * reserved, so it only reaches the parser through this production - a quoted "_"
 * is an ordinary identifier. Only meaningful as a direct arg of a lambda call,
 * rejected anywhere else during parse analysis.
 */
typedef struct HolePlaceholder
{
	NodeTag		type;
	ParseLoc	location;		/* token location, or -1 if unknown */
} HolePlaceholder;
