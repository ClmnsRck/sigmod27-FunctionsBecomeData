...
typedef enum ExprEvalOp
{
	...
	/* evaluate LambdaFuncAlias(dispatch jump to injected lambda) */
	EEOP_LAMBDAFUNCALIAS,

	/* 
	 * Evaluate CallLambdaExpr
	 * 
	 * This step loads the lambda expressions from Var, compiles it,
	 * then runs the created steps on the results from its inputs.
	 */
	EEOP_CALLLAMBDAEXPR,
...
typedef struct ExprEvalStep
{
	...
	union
	{
		...
		/* for EEOP_LAMBDAFUNCALIAS */
		struct
		{
			int			jumpdone;		//Where to jump for the current injection
			int			nargs;			//how many args
			Datum	   *argvalues;		//pointer to inputs
			bool	   *argnulls;		//are inputs null
			/* only for injection purposes */
			int			rowID;
			int			colID;
		} 			lambdafuncalias;

		/* for EEOP_CALLLAMBDAEXPR */
		struct
		{
			CallLambdaExprState *cle_state;
		} 			calllambdaexpr;
		...
		struct
		{
			...
			/* cached descriptor for composite type - filled at runtime */
			ExprEvalRowtypeCache rowcache;
			bool lambda; 			/* determine if inside lambdas */
		}			fieldselect;
 ...
 */
extern void ExecEvalCallLambdaExpr(ExprState *state, ExprEvalStep *op,
								   ExprContext *econtext);
