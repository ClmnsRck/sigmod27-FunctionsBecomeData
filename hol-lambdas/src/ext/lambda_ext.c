...
static TupleDesc label_record_type(List *args)
{
    LambdaExpr *lambda = (LambdaExpr *)list_nth(args, 1);
    TupleDesc inDesc = (TupleDesc) list_nth(lambda->argTypesTupDesc, 0);
    TupleDesc outDesc;
    outDesc = CreateTemplateTupleDesc(inDesc->natts + 1);
    ...
    label_internal(PG_FUNCTION_ARGS)
{
    ...
    const TupleTableSlotOps *slotOps = &TTSOpsHeapTuple;
    TupleTableSlot *slot = MakeTupleTableSlot(NULL, slotOps);
...
static Datum label_fast_internal(PG_FUNCTION_ARGS, Datum (*evalfunc)(Datum **arg), bool cursor)
{
    MemoryContext oldcontext;
    MemoryContext per_query_ctx;
    TupleDesc outDesc = NULL;
    ...
    TupleTableSlot *slot = MakeTupleTableSlot(NULL, &TTSOpsHeapTuple);
    Tuplestorestate *tsOut = tuplestore_begin_heap(true, false, work_mem);
    ...
    if (!cursor)
    {
        TupleTableSlot *slotCursor = MakeTupleTableSlot(inDesc, &TTSOpsMinimalTuple);
        bool shouldFree = false;
        while (tuplestore_gettupleslot(ttsIn, true, false, slotCursor))
        {
            heap_deform_tuple(heap_tuple_from_minimal_tuple(ExecFetchSlotMinimalTuple(slotCursor, &shouldFree)), 
                            inDesc, oldVal, oldIsNull);
    ...
    else
    {
        for (slot = ExecProcNode(planState); !TupIsNull(slot); slot = ExecProcNode(planState))
        {
            ...
            //     heap_deform_tuple(tuple, inDesc, val_ptr, null_ptr);
            // }
            // else
            // {
            //     slot_getallattrs(slot);
            ...
            //     hdr = slot->tts_tuple->t_data;
            // }
            heap_deform_tuple(slot->tts_ops->get_heap_tuple(slot), inDesc, oldVal, oldIsNull);
    ...
    label_fast(PG_FUNCTION_ARGS)
{
    ...
    else if (list_length(lambda->argnames) != 1)
    {
        ereport(ERROR,
                (errcode(ERRCODE_INTERNAL_ERROR),
