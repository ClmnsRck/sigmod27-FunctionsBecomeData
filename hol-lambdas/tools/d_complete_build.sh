...
#setup project
./configure --with-llvm --prefix="$MY_ROOT_DIR/install" CFLAGS='-fopenmp -Wno-ignored-attributes' CXXFLAGS='-lpthread -lpq -lm -fopenmp' --enable-debug --enable-cassert
...
#The build is configured to build with low optimisation and debug symbols