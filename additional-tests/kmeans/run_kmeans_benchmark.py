...
def required_paths(cfg: RunnerConfig, type_: str) -> Tuple[str, str]:
    setup_sql = os.path.join(cfg.setup_scripts_dir, "setup_kmeans.sql")

    if type_ == "kmeans_udf":
        bench_sql = os.path.join(cfg.benchmarks_dir, "benchmark_kmeans_udf.sql")
    elif type_ == "kmeans_sql":
        bench_sql = os.path.join(cfg.benchmarks_dir, "benchmark_kmeans_sql.sql")
    else:
        raise ValueError(f"Unknown type {type_!r} (expected kmeans_udf|kmeans_sql)")
...
def main() -> None:
    ...
    ap.add_argument("--types", nargs="*", default=["kmeans_udf", "kmeans_sql"],
                    help="Which benchmark sets to run: kmeans_udf kmeans_sql (default: all)")
