...
--------------------------------------------------------- load functions -----------------------------------------------------------
create or replace function apply_mt_agg("lambda", lambdatable, lambdatable, int, int, int)
returns setof record
as '$libdir/apply_mt_agg_ext.so','apply_mt_agg'
language C STRICT;
----------------------------------------------------------------------------------------------------------------------------------------
...
DECLARE
  ...
  queue_cap   int := 0;
BEGIN
  FOR repeat_body IN 1..50 LOOP   -- <-- scale work here
    -- build "(expr) + (expr) + ...", repeated repeat_body times
    SELECT string_agg(format('(%s)', 'b.g(a.x, a.y, a.z, b.f) + a.z'), ' + ')
      INTO body
    FROM generate_series(1, repeat_body);

    FOR run_ctr IN 1..10 LOOP
      DISCARD PLANS;

      sql := format($fmt$
        SELECT count(*) FROM apply_mt_agg(
      ...
      $fmt$, body, ndata, nthreads, queue_cap, repeat_body);

      RAISE NOTICE 'repeat_body=%, run=%/% (ndata=%, nthreads=%, queuecap=%)',
                    repeat_body, run_ctr, 10, ndata, nthreads, queue_cap;
