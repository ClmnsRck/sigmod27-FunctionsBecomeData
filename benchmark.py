...
class RunnerConfig:
    ...
    setup_scripts_dir: str = "setup-scripts"
...
def extract_jit_timing(jit_info: Optional[dict]) -> Tuple[float, float, float, float, float]:
    """
    Pull the five JIT timing fields (in ms) out of the JIT section returned
...
def required_paths(cfg: RunnerConfig, type_: str) -> Tuple[str, str]:
    setup_sql = os.path.join(cfg.setup_scripts_dir, "setup.sql")

    if type_ == "hol":
        bench_sql = os.path.join(cfg.benchmarks_dir, "benchmark_hol.sql")
    elif type_ == "baseline":
        bench_sql = os.path.join(cfg.benchmarks_dir, "benchmark_baseline.sql")
    elif type_ == "cores":
        bench_sql = os.path.join(cfg.benchmarks_dir, "benchmark_cores.sql")
    elif type_ == "baseline_cores":
        bench_sql = os.path.join(cfg.benchmarks_dir, "benchmark_baseline_cores.sql")
    else:
        raise ValueError(f"Unknown type {type_!r} (expected hol|baseline|cores|baseline_cores)")
...
def benchmark_repo_test(
    ...
    with open(frontend_log, "w", buffering=1, encoding="utf-8") as fe:
        ...
        try:
            ...
            # make sure planner stats are stable.
            ...
            with open(csv_path, "w", newline="", encoding="utf-8") as out_fh:
                ...
                for ch in chunks:
                    ...
                    # Cap PostgreSQL's parallel workers to this chunk's thread budget 
                    pgather = 0 if ch.nthreads <= 1 else ch.nthreads
                    try:
                        with conn.cursor() as cur:
                            cur.execute(f"SET max_parallel_workers_per_gather = {pgather};")
                    except Exception:
                        pass
...
def main() -> None:
    ...
    ap.add_argument("--types", nargs="*", default=["hol", "baseline", "cores", "baseline_cores"],
                    help="Which benchmark sets to run: hol baseline cores baseline_cores (default: all)")
